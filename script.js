...
const DB_NAME = "montage-ia-mobile-final-pro";
const DB_VERSION = 1;
...
const VOICE_STYLE_GROUPS = {
  naturel: [
    { id: "masculin-naturel", label: "Masculin naturel", gender: "male" },
    { id: "feminin-naturel", label: "Féminin naturel", gender: "female" },
    { id: "masculin-mature", label: "Masculin mature", gender: "male" },
    { id: "feminin-mature", label: "Féminin mature", gender: "female" }
  ],
  emotion: [
    { id: "masculin-emotion", label: "Masculin émotion", gender: "male" },
    { id: "feminin-emotion", label: "Féminin émotion", gender: "female" },
    { id: "voix-douce", label: "Voix douce", gender: "female" },
    { id: "voix-sombre", label: "Voix sombre", gender: "male" }
  ],
  dynamique: [
    { id: "masculin-energetique", label: "Masculin énergique", gender: "male" },
    { id: "feminin-dynamique", label: "Féminin dynamique", gender: "female" },
    { id: "voix-punchy", label: "Voix punchy", gender: "male" },
    { id: "voix-annonce", label: "Voix annonce", gender: "female" }
  ],
  special: [
    { id: "voix-robot", label: "Voix robot", gender: "male" },
    { id: "voix-ia-futuriste", label: "Voix IA futuriste", gender: "female" },
    { id: "voix-mysterieuse", label: "Voix mystérieuse", gender: "female" },
    { id: "voix-froide", label: "Voix froide", gender: "male" }
  ]
};
...
function findVoiceStyle(styleId) {
  const all = Object.values(VOICE_STYLE_GROUPS).flat();
  return all.find((item) => item.id === styleId) || VOICE_STYLE_GROUPS.naturel[0];
}
...
function getRelevantMedia(kind, mediaType) {
  return state.cache.media.filter((item) => item.bucket === `${kind}-${mediaType}`);
}

function selectedCountLabel(selectedIds = []) {
  return `${selectedIds.length} média${selectedIds.length > 1 ? "s" : ""} lié${selectedIds.length > 1 ? "s" : ""}`;
}

function renderSelectableMediaCards(kind, mediaType, selectedIds = []) {
  const items = getRelevantMedia(kind, mediaType);

  if (!items.length) {
    return `<div class="empty-state small"><p>Aucun média disponible dans cette section.</p></div>`;
  }
  ...
  return `
    <div class="media-grid">
      ${items.map((item) => {
        ...
        return `
          <button type="button" class="media-card" data-action="toggle-project-media" data-kind="${kind}" data-media-type="${mediaType}" data-media-id="${item.id}">
            ${item.mediaType === "image"
              ? `<img src="${URL.createObjectURL(item.blob)}" alt="${escapeHtml(item.fileName)}" />`
              : `<video src="${URL.createObjectURL(item.blob)}" muted playsinline></video>`
            }
            <div class="media-card-label">${isSelected ? "✓ " : ""}${escapeHtml(item.fileName)}</div>
          </button>
        `;
      }).join("")}
    </div>
...
function musicProjectTemplate() {
  ...
  const selectedLabel = selectedCountLabel(selectedMediaIds);

  return panel(
    "Projet musique",
    "Musique + médias liés au projet. La vraie timeline manuelle sera branchée sur cette base.",
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="result-box">
          <div class="result-box-head">
            <h3>Médias liés au projet</h3>
            <button class="chip-btn" type="button" data-action="open-library" data-mode="music" data-type="${draft.mode}">Bibliothèque</button>
          </div>
          <p class="small-note">${selectedLabel}</p>
          ${renderSelectableMediaCards("music", draft.mode, selectedMediaIds)}
        </div>
...
function speechProjectTemplate() {
  ...
  return panel(
    "Projet speech",
    "Voix mieux rangées + métadonnées + médias liés au projet.",
    `
      <form id="speechProjectForm" class="stack-form">
        ...
        <div class="result-box">
          ...
          <p class="small-note">${selectedLabel}</p>
          ${renderSelectableMediaCards("speech", draft.mode, selectedMediaIds)}
        </div>
...
function libraryTemplate() {
  ...
  return panel(
    "Bibliothèque",
    "Bibliothèque locale persistante par profil.",
    `
...
function projectsTemplate() {
  return panel(
    "Mes projets",
    "Reprise propre des projets et de leurs médias liés.",
    `
...
function resultTemplate() {
  ...
  const linkedCount = project.config?.selectedMediaIds?.length || 0;

  return panel(
    "Résultat du projet",
    "Projet rouvert depuis le stockage local.",
    `
      <div class="result-box">
        <div class="result-box-head">
          <h3>Infos projet</h3>
        </div>
        <p class="small-note">Type : ${project.type === "music" ? "Musique" : project.type === "speech" ? "Speech" : "Sora"}</p>
        <p class="small-note">Médias liés : ${linkedCount}</p>
      </div>
...
async function createMusicProject() {
  ...
  const hashtags = makeHashtags([
    ...
    "Clip",
    draft.style,
    draft.mode === "image" ? "PhotoEdit" : "VideoEdit"
  ]).slice(0, 5);

  const mainMeta = buildMainMeta({
    title: `${name} ${draft.style}`,
    description: `Montage ${draft.mode === "image" ? "image" : "vidéo"}, ambiance ${draft.style}, prêt à publier.`,
    hashtags
  });
...
async function generateSpeechContentComplete() {
  ...
  try {
    const voiceStyleInfo = findVoiceStyle(draft.voiceStyle);
    ...
    const [metaResult, speechResponse] = await Promise.all([
      postJson("/api/meta/generate", {
        ...
        notes: `Style vocal ${voiceStyleInfo.label}. Réponse en un seul bloc, sans écrire Titre Description Hashtags, 5 hashtags max.`
      }),
...
async function createSpeechProject() {
  ...
  const fallbackHashtags = makeHashtags([
    "Speech",
    "Narration",
    draft.tone,
    draft.mode,
  ...
  const mainMeta =
    ...
    buildMainMeta({
      title: `${name} ${voiceStyleInfo.label}`,
      description: `${draft.mode === "image" ? "Montage image" : "Montage vidéo"}, ton ${draft.tone}, voix ${voiceStyleInfo.label.toLowerCase()}.`,
...
async function loadSpeechProjectIntoDraft(project) {
  state.temp.speechDraft = {
    ...
    voiceStyle: project.config?.voiceStyle || "masculin-naturel",
  ...
  state.temp.speechDraft.voiceStyle = ensureStyleInFamily(
    state.temp.speechDraft.voiceFamily,
    state.temp.speechDraft.voiceStyle
  );
  ...
  state.cache.projects.forEach((project) => {
    ...
    wrapper.innerHTML = `
      <article class="project-card">
        ...
        <div class="small-note">
          Médias liés : ${(project.config?.selectedMediaIds || []).length}
        </div>
