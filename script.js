...
const DB_NAME = "montage-ia-mobile-v4";
const DB_VERSION = 1;
...
function montageModeHelp(mode) {
  return mode === "manual"
    ? "Tu choisis l’ordre des images et vidéos avant le rendu."
    : "L’application place les médias toute seule pour créer la vidéo.";
}

function sourceModeHelp(mode) {
  if (mode === "single") return "L’IA et le projet utilisent un seul bloc précis.";
  if (mode === "multi") return "L’IA et le projet utilisent plusieurs blocs choisis par toi.";
  return "L’IA peut chercher dans plusieurs blocs compatibles avec le projet.";
}
...
function getDraftBlocks(draft) {
  ...
  if (draft.mediaSourceMode === "single") {
    return [normalizeBlock(draft.primaryBlock)];
  }
...
function buildMusicGeminiContext(draft) {
  ...
  return [
    `format final : ${aspectRatioLabel(draft.aspectRatio)}`,
    `source médias : ${sourceModeLabel(draft.mediaSourceMode)}`,
    `blocs autorisés : ${blocks.join(", ")}`,
    `style : ${draft.style}`,
    `mode médias : ${draft.mode === "video" ? "vidéos" : "images"}`
  ].join(" | ");
}
...
function renderMusicAnalysisHtml(analysis) {
  if (!analysis) {
    return `<div class="empty-state">Aucune analyse Gemini pour le moment.</div>`;
  }
  ...
  return `
    <div class="card-list">
      <div class="project-card">
        <h3 class="project-title">Résumé</h3>
        <div class="small-note">${escapeHtml(analysis.summary || "Aucun résumé.")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Ambiance</h3>
        <div class="small-note">${escapeHtml(analysis.dominantMood || "Non détectée")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Énergie</h3>
        <div class="small-note">${escapeHtml(analysis.energyLevel || "Non détectée")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Rythme</h3>
        <div class="small-note">${escapeHtml(analysis.rhythmEstimate || "Non détecté")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Univers visuel</h3>
        <div class="small-note">${escapeHtml(analysis.visualUniverse || "Non détecté")}</div>
      </div>

      ${emotions.length ? `
        <div class="project-card">
          <h3 class="project-title">Émotions</h3>
          <div class="small-note">${emotions.map((item) => escapeHtml(item)).join(" • ")}</div>
        </div>
      ` : ""}

      ${sceneIdeas.length ? `
        <div class="project-card">
          <h3 class="project-title">Idées de scènes</h3>
          <div class="small-note">${sceneIdeas.map((item) => `- ${escapeHtml(item)}`).join("<br>")}</div>
        </div>
      ` : ""}

      ${editingAdvice.length ? `
        <div class="project-card">
          <h3 class="project-title">Conseils montage</h3>
          <div class="small-note">${editingAdvice.map((item) => `- ${escapeHtml(item)}`).join("<br>")}</div>
        </div>
      ` : ""}

      ${hookMoments.length ? `
        <div class="project-card">
          <h3 class="project-title">Moments forts</h3>
          <div class="small-note">${hookMoments.map((item) => `- ${escapeHtml(item)}`).join("<br>")}</div>
        </div>
      ` : ""}

      ${analysis.soraDirection ? `
        <div class="project-card">
          <h3 class="project-title">Direction clip</h3>
          <div class="small-note">${escapeHtml(analysis.soraDirection)}</div>
        </div>
      ` : ""}
    </div>
  `;
...
function renderMusicClipIdeasHtml(ideas) {
  if (!ideas) {
    return `<div class="empty-state">Aucune idée de clip pour le moment.</div>`;
  }
  ...
  return `
    <div class="card-list">
      <div class="project-card">
        <h3 class="project-title">Direction créative</h3>
        <div class="small-note">${escapeHtml(ideas.creativeDirection || "Aucune direction.")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Style visuel</h3>
        <div class="small-note">${escapeHtml(ideas.visualStyle || "Non défini")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Caméra</h3>
        <div class="small-note">${escapeHtml(ideas.cameraStyle || "Non défini")}</div>
      </div>

      <div class="project-card">
        <h3 class="project-title">Palette</h3>
        <div class="small-note">${escapeHtml(ideas.colorPalette || "Non définie")}</div>
      </div>

      ${storyArc.length ? `
        <div class="project-card">
          <h3 class="project-title">Structure du clip</h3>
          <div class="small-note">${storyArc.map((item) => `- ${escapeHtml(item)}`).join("<br>")}</div>
        </div>
      ` : ""}

      ${shortPromptIdeas.length ? `
        <div class="project-card">
          <h3 class="project-title">Idées rapides</h3>
          <div class="small-note">${shortPromptIdeas.map((item) => `- ${escapeHtml(item)}`).join("<br>")}</div>
        </div>
      ` : ""}
    </div>
  `;
    ...
    <section class="screen">
      ...
      <div class="action-grid">
        ...
        <button class="action-card" data-action="go-route" data-route="musicProject">
          ...
          <span class="action-desc">Audio + blocs + format + vidéo</span>
        </button>
...
function musicProjectTemplate() {
  ...
  return panel(
    "Montage musique",
    "1. Ajoute la musique - 2. Choisis le format - 3. Choisis la source des médias - 4. Analyse avec Gemini - 5. Crée la vidéo.",
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <label class="field">
          <span>Durée finale de la vidéo</span>
          <select id="musicDuration">
            <option value="10" ${draft.targetDuration === "10" ? "selected" : ""}>10 secondes</option>
        ...
        <div class="sticky-actions">
          <button id="musicAnalyzeBtn" class="secondary-btn" type="button">
            ${state.temp.musicAnalyzing ? "Analyse..." : "Analyser la musique"}
          </button>
          <button id="musicIdeasBtn" class="secondary-btn" type="button">
            ${state.temp.musicIdeasLoading ? "Création..." : "Créer idée de clip"}
          </button>
...
function speechProjectTemplate() {
  ...
  return panel(
    "Voix IA",
    "1. Écris ton texte - 2. Choisis la voix - 3. Choisis le format - 4. Choisis la source des médias - 5. Génère.",
    `
...
function libraryTemplate() {
  return panel(
    "Bibliothèque médias",
    "Ajoute des images et vidéos, classe-les par bloc, et laisse l’app détecter leur orientation.",
    `
        ...
        <div class="row-2">
          ...
          <label class="field">
            ...
            <select id="libraryBlockFilter">
              ...
              ${allMediaBlocks().map((block) => `
                <option value="${escapeHtml(block)}" ${state.libraryBlockFilter === block ? "selected" : ""}>${escapeHtml(block)}</option>
              `).join("")}
            </select>
          </label>
        </div>
...
function resultTemplate() {
  ...
  const geminiAnalysis = project.config?.geminiAnalysis || null;
  const geminiClipIdeas = project.config?.geminiClipIdeas || null;
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        <p class="small-note">Source des médias : ${sourceModeLabel(project.config?.mediaSourceMode || "single")}</p>
      ...
      ${project.type === "music" ? `
        <div class="result-box">
          ...
          ${renderMusicAnalysisHtml(geminiAnalysis)}
        </div>
        ...
        <div class="result-box">
          ...
          ${renderMusicClipIdeasHtml(geminiClipIdeas)}
        </div>
      ` : ""}
      ...
      <div class="sticky-actions">
        ${canRender ? `
          <button class="secondary-btn" type="button" data-action="render-project-video" data-id="${project.id}" ${isBusy ? "disabled" : ""}>
            ${isBusy ? "Rendu en cours..." : renderStatus === "done" ? "Refaire la vidéo" : "Créer la vidéo"}
          </button>
        ` : ""}
        ${finalVideo ? `
...
async function analyzeMusicWithGemini() {
  ...
  try {
    ...
    formData.append("target", "clip musical court de 30 secondes maximum");
    formData.append("context", buildMusicGeminiContext(draft));

    const result = await postGeminiForm("/api/gemini/analyze-audio", formData);

    if (result?.analysis) {
      state.temp.musicAnalysis = result.analysis;
    } else if (result?.raw) {
      state.temp.musicAnalysis = { raw: result.raw };
    } else {
...
async function generateMusicClipIdeas() {
  ...
  try {
    ...
    if (result?.result) {
      state.temp.musicClipIdeas = result.result;
    } else if (result?.raw) {
      state.temp.musicClipIdeas = { raw: result.raw };
    } else {
      throw new Error("Idée de clip vide.");
    }
...
function bindSpeechProject() {
  ...
  [nameInput, textInput, styleSelect, toneSelect, speedSelect, montageModeSelect, aspectRatioSelect].forEach((el) => {
    el?.addEventListener("input", syncDraft);
    el?.addEventListener("change", syncDraft);
  });
