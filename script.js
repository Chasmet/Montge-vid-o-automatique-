...
const DB_NAME = "montage-ia-mobile-v7";
const DB_VERSION = 1;
...
function subtitleModeLabel(mode) {
  return mode === "auto" ? "Sous-titres auto" : "Sans sous-titres";
}
...
function pickVariedMediaIds(candidates, limit = 6) {
  const items = shuffleArray(Array.isArray(candidates) ? candidates : []);
  ...
  for (const item of items) {
    ...
    const signature = `${block}|${mediaSignature(item)}`;

    if (!byBlock.has(block)) {
      byBlock.set(block, true);
  ...
  return uniqueArray(selected).slice(0, limit);
}

function buildTitleVariants(title, mood, visual, style) {
  const cleanTitle = safeText(title) || "Projet vidéo";
  const cleanMood = safeText(mood || "").toLowerCase();
  const cleanVisual = safeText(visual || "").toLowerCase();
  const cleanStyle = safeText(style || "").toLowerCase();

  const variants = [
    cleanTitle,
    `${cleanTitle} - version ${cleanMood || "ciné"}`,
    `${cleanTitle} | clip ${cleanVisual || cleanStyle || "créatif"}`,
    `${cleanTitle} | ambiance ${cleanMood || "forte"}`,
    `${cleanTitle} - montage ${cleanStyle || "ia"}`
  ];

  return uniqueArray(variants.map((v) => v.trim()).filter(Boolean));
}

function buildHashtags({ mood, visual, style, formatTag, baseTag = "#montageia" }) {
  return uniqueArray([
    baseTag,
    "#clipvideo",
    mood ? `#${slugTag(mood)}` : "",
...
function buildLocalMusicMeta(projectName, draft, analysis, clipIdeas) {
  const titleBase = safeText(projectName) || "Clip musique IA";
  ...
  const subtitleText = subtitleModeLabel(draft?.subtitleMode || "none");

  const title = shuffleArray(buildTitleVariants(titleBase, mood, visual, draft?.style))[0];

  const descriptionLines = [
    summary,
    `Format ${formatText}, montage en ${mediaText}, ambiance ${mood}.`,
    visual ? `Univers visuel : ${visual}.` : "",
    camera ? `Caméra : ${camera}.` : "",
    `Sous-titres : ${subtitleText}.`
  ].filter(Boolean);
  ...
  const hashtags = buildHashtags({
    ...
    visual,
    style: draft?.style,
    formatTag: draft?.aspectRatio === "vertical" ? "#shortvideo" : "#cinematique",
    baseTag: "#clipmusical"
  });

  const shortTitle = shuffleArray(buildTitleVariants(titleBase, mood, visual, "shorts"))[0];

  const general = `${title}
${descriptionLines.join(" ")}
${hashtags.join(" ")}`;

  const shorts = `${shortTitle}
${buildHashtags({
    mood,
    visual,
    style: "shorts",
...
function buildLocalSpeechMeta(projectName, draft) {
  const titleBase = safeText(projectName) || "Voix IA";
  const voiceLabel = findVoiceLabel(draft?.voiceStyle || "masculin-naturel");
  const tone = safeText(draft?.tone || "normal");
  const formatText = aspectRatioLabel(draft?.aspectRatio || "vertical");
  const subtitleText = subtitleModeLabel(draft?.subtitleMode || "none");

  const title = shuffleArray(buildTitleVariants(titleBase, tone, voiceLabel, "narration"))[0];
...
${buildHashtags({
    ...
    visual: voiceLabel,
    style: "narration",
    formatTag: draft?.aspectRatio === "vertical" ? "#shortvideo" : "#video",
    baseTag: "#voixia"
  }).join(" ")}`;
...
function getDraftBlocks(draft) {
  ...
  if (draft.mediaSourceMode === "multi") {
    return (draft.allowedBlocks || []).map(normalizeBlock).filter(Boolean);
  }
...
function buildMusicGeminiContext(draft) {
  ...
  return [
    ...
    `sous_titres=${subtitleModeLabel(draft.subtitleMode || "none")}`
  ].join(" | ");
}
...
async function loadSpeechDraft() {
  ...
  state.temp.speechDraft.subtitleMode = state.temp.speechDraft.subtitleMode || "none";
...
async function loadMusicDraft() {
  ...
  state.temp.musicDraft.subtitleMode = state.temp.musicDraft.subtitleMode || "none";
    ...
    <section class="screen">
      ...
      <div class="action-grid">
        ...
        <button class="action-card" data-action="open-library" data-mode="music" data-type="image">
          ...
          <span class="action-desc">Images et vidéos classées</span>
        </button>
...
function renderSubtitleOptions(selected) {
  return `
    <option value="none" ${selected === "none" ? "selected" : ""}>Sans sous-titres</option>
    <option value="auto" ${selected === "auto" ? "selected" : ""}>Sous-titres auto</option>
  `;
}
      ...
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="row-2">
          <label class="field">
            ...
            <select id="musicSubtitleMode">
              ${renderSubtitleOptions(draft.subtitleMode || "none")}
            </select>
          </label>
          <div></div>
        </div>
...
function libraryTemplate() {
  return panel(
    "Bibliothèque médias",
    "Simple : 1. choisis la catégorie, 2. choisis images ou vidéos, 3. choisis le bloc, 4. ajoute tes fichiers.",
    `
      <div class="stack-form">
        <div class="result-box">
          <div class="result-box-head"><h3>Aide rapide</h3></div>
          <div class="small-note">
            - Musique = médias pour clips musique<br>
            - Voix = médias pour narration / texte<br>
            - Bloc = dossier simple pour ranger tes médias<br>
            - Filtre = voir seulement un dossier
          </div>
        </div>
        ...
        <div class="prompt-actions">
          <button type="button" data-action="create-custom-block">Créer un nouveau bloc</button>
        </div>
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        <p class="small-note">Sous-titres : ${subtitleModeLabel(project.config?.subtitleMode || "none")}</p>
...
function bindMusicProject() {
  ...
  const subtitleModeSelect = document.getElementById("musicSubtitleMode");
  ...
  const syncDraft = () => {
    ...
    state.temp.musicDraft = {
      ...
      subtitleMode: subtitleModeSelect?.value || "none"
    };
  ...
  [nameInput, durationSelect, styleSelect, montageModeSelect, aspectRatioSelect, subtitleModeSelect].forEach((el) => {
...
async function syncMusicAiToExistingProject() {
  ...
  await projectPut({
    ...
    config: {
      ...
      montagePlan: state.temp.musicMontagePlan || null,
      subtitleMode: state.temp.musicDraft.subtitleMode || "none"
    }
  });
  ...
  try {
    ...
    try {
      metaResult = await postJson("/api/meta/generate", {
        ...
        analysis: state.temp.musicAnalysis,
        clipIdeas: state.temp.musicClipIdeas || null,
        subtitleMode: draft.subtitleMode || "none"
      });
    } catch (error) {
...
async function pilotMusicProjectWithAi() {
  ...
  try {
    ...
    try {
      ...
      console.warn("Sélection IA indisponible, fallback local:", error);
    }

    if (!selectedMediaIds.length) {
      selectedMediaIds = pickVariedMediaIds(candidates, Math.min(6, candidates.length));
    } else {
      const selectedCandidatesFromAi = candidates.filter((item) => selectedMediaIds.includes(item.id));
      selectedMediaIds = pickVariedMediaIds(selectedCandidatesFromAi, Math.min(6, selectedCandidatesFromAi.length || selectedMediaIds.length));
    }
    ...
    if (Array.isArray(plan?.selectedMediaIds) && plan.selectedMediaIds.length) {
      selectedMediaIds = uniqueArray(plan.selectedMediaIds);
    }
...
async function createMusicProject(options = {}) {
  ...
  let selectedMediaIds = draft.selectedMediaIds || [];
  if (!selectedMediaIds.length) {
    selectedMediaIds = pickVariedMediaIds(mapMediaCandidatesForAi("music", draft), 6);
    state.temp.musicDraft.selectedMediaIds = selectedMediaIds;
  }
...
async function loadMusicProjectIntoDraft(project) {
  state.temp.musicDraft = {
    ...
    subtitleMode: project.config?.subtitleMode || "none"
  };
...
function bindSpeechProject() {
  ...
  const subtitleModeSelect = document.getElementById("speechSubtitleMode");
  ...
  [nameInput, textInput, styleSelect, toneSelect, speedSelect, montageModeSelect, aspectRatioSelect, durationSelect, subtitleModeSelect].forEach((el) => {
    el?.addEventListener("input", syncDraft);
    el?.addEventListener("change", syncDraft);
  });
...
async function generateSpeechContentComplete() {
  ...
  try {
    ...
    const localMeta = buildLocalSpeechMeta(title, draft);

    resetObjectUrl(state.temp.speechAudioUrl);
    state.temp.speechAudioBlob = audioBlob;
    state.temp.speechAudioUrl = URL.createObjectURL(audioBlob);
    state.temp.speechMetaGeneral = safeText(metaResult?.general || localMeta.general);
    state.temp.speechMetaShorts = safeText(metaResult?.shorts || localMeta.shorts).slice(0, 99);
...
async function createSpeechProject() {
  ...
  const previous = draft.id ? state.cache.projects.find((p) => p.id === draft.id) : null;
  const localMeta = buildLocalSpeechMeta(name, draft);
  ...
  const project = {
    ...
    mainMeta: state.temp.speechMetaGeneral || localMeta.general,
    shortMeta: state.temp.speechMetaShorts || localMeta.shorts
  };
...
async function renderProjectVideo(projectId) {
  ...
  try {
    ...
    formData.append("subtitleMode", project.config?.subtitleMode || "none");
