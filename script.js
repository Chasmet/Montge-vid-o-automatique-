...
function montageModeHelp(mode) {
  return mode === "manual"
    ...
    : "L’application prépare tout et peut lancer le rendu seule.";
}
...
function sourceModeHelp(mode) {
  ...
  return "L’IA peut choisir elle-même les blocs compatibles.";
}
...
function getPlanSelectedIds(plan) {
  if (!plan) return [];
  if (Array.isArray(plan.selectedMediaIds) && plan.selectedMediaIds.length) {
    return uniqueIds(plan.selectedMediaIds);
  }
  if (Array.isArray(plan.timeline) && plan.timeline.length) {
    return uniqueIds(plan.timeline.map((item) => item?.mediaId));
  }
  return [];
}

function getProjectSelectedMediaIds(project) {
  if (!project) return [];

  const direct = uniqueIds(project.config?.selectedMediaIds || []);
  if (direct.length) return direct;

  const planIds = getPlanSelectedIds(project.config?.montagePlan || project.config?.plan || null);
  if (planIds.length) return planIds;

  return [];
}

function getRecentMusicUsedIds(excludeProjectId = null, limitProjects = 8) {
  ...
  projects.forEach((project) => {
    ids.push(...getProjectSelectedMediaIds(project));
  });

  return uniqueIds(ids);
}

function chooseLocalDiverseMediaCandidates(kind, draft, incomingCandidates = [], limit = 6) {
  const baseCandidates = Array.isArray(incomingCandidates) && incomingCandidates.length
    ? incomingCandidates
    : mapMediaCandidatesForAi(kind, draft);

  if (!baseCandidates.length) return [];

  const maxCount = Math.max(1, Math.min(limit, baseCandidates.length));
  const recentIds = new Set(getRecentMusicUsedIds(draft?.id || null));
  const poolsByBlock = new Map();

  baseCandidates.forEach((item) => {
    const block = normalizeBlock(item.block || item.collection || "Vrac");
    if (!poolsByBlock.has(block)) poolsByBlock.set(block, []);
    poolsByBlock.get(block).push(item);
  });

  const chosen = [];
  const chosenIds = new Set();

  const blockNames = shuffleArray([...poolsByBlock.keys()]);

  for (const blockName of blockNames) {
    if (chosen.length >= maxCount) break;

    const pool = shuffleArray(poolsByBlock.get(blockName) || []);
    const nonRecent = pool.filter((item) => !recentIds.has(item.id));
    const recent = pool.filter((item) => recentIds.has(item.id));
    const pick = nonRecent[0] || recent[0] || null;

    if (pick && !chosenIds.has(pick.id)) {
      chosen.push(pick);
...
function getSelectedOrFallbackMedia(project) {
  ...
  const selectedIds = getProjectSelectedMediaIds(project);
...
function buildLocalMusicMeta(projectName, draft, analysis, clipIdeas) {
  const title = safeText(projectName) || "Clip musique IA";
  const summary = safeText(
    analysis?.summary ||
    clipIdeas?.creativeDirection ||
    "Clip musical créé automatiquement avec analyse IA."
  );
  const mood = safeText(analysis?.dominantMood || draft?.style || "emotion");
  ...
  const formatText = aspectRatioLabel(draft?.aspectRatio || "vertical");
  const mediaText = draft?.mode === "video" ? "vidéos" : "images";

  const descriptionParts = [
    summary,
    `Montage automatique en ${mediaText}, format ${formatText}.`,
...
function renderSubtitlesHtml(subtitles) {
  if (!subtitles || !subtitles.enabled) {
    return `<div class="empty-state">Sous-titres désactivés ou non générés.</div>`;
  }

  const segments = Array.isArray(subtitles.segments) ? subtitles.segments : [];

  if (!segments.length) {
    return `<div class="empty-state">Sous-titres vides.</div>`;
  }

  return `
    <div class="card-list">
      <div class="project-card">
        <h3 class="project-title">Sous-titres préparés</h3>
        <div class="small-note">${segments.length} segments</div>
      </div>
      <div class="project-card">
        <h3 class="project-title">Aperçu</h3>
        <div class="small-note">
          ${segments
            .slice(0, 8)
            .map((item) => `${formatSeconds(item.start)} - ${formatSeconds(item.end)} : ${escapeHtml(item.text || "")}`)
            .join("<br>")}
        </div>
      </div>
    </div>
...
async function loadMusicDraft() {
  ...
  state.temp.musicDraft.includeSubtitles = !!state.temp.musicDraft.includeSubtitles;
...
function renderMusicAnalysisHtml(analysis) {
  if (!analysis) return `<div class="empty-state">Aucune analyse Gemini.</div>`;
...
function renderMusicMontagePlanHtml(plan) {
  ...
  return `
    <div class="card-list">
      <div class="project-card"><h3 class="project-title">Transition</h3><div class="small-note">${escapeHtml(plan.transitionStyle || "cut")}</div></div>
      <div class="project-card"><h3 class="project-title">Effet</h3><div class="small-note">${escapeHtml(plan.effectStyle || "clean")}</div></div>
      <div class="project-card"><h3 class="project-title">Médias retenus</h3><div class="small-note">${Array.isArray(plan.selectedMediaIds) ? plan.selectedMediaIds.join(" • ") : "—"}</div></div>
      ${timeline.length ? `
        <div class="project-card">
...
function musicProjectTemplate() {
  ...
  return panel(
    "Montage musique",
    "Analyse Gemini, idée de clip, métadonnées, pilotage IA, puis rendu vidéo.",
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="result-box">
          <div class="result-box-head"><h3>Options Pilotage IA</h3></div>
          <label class="project-card" style="display:flex;align-items:flex-start;gap:12px;">
            <input id="musicIncludeSubtitles" type="checkbox" ${draft.includeSubtitles ? "checked" : ""} />
            <div>
              <div class="project-title">Sous-titres automatiques</div>
              <div class="small-note">À activer seulement si tu veux préparer des sous-titres pour ce projet.</div>
            </div>
          </label>
        </div>
        ...
        <div class="sticky-actions">
          ...
          <button id="musicMetaBtn" class="secondary-btn" type="button">${state.temp.musicMetaLoading ? "Méta..." : "Créer les métadonnées"}</button>
          <button id="musicPilotBtn" class="secondary-btn" type="button">${state.temp.musicPilotLoading ? "Pilotage..." : "Pilotage IA complet"}</button>
          <button class="primary-btn" type="submit">Créer le projet vidéo</button>
        </div>
      </form>
      ...
      <div class="result-box">
        <div class="result-box-head"><h3>Sous-titres IA</h3></div>
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        <p class="small-note">Médias liés : ${getProjectSelectedMediaIds(project).length}</p>
        ${project.type === "music" ? `<p class="small-note">Sous-titres : ${project.config?.includeSubtitles ? "Activés" : "Désactivés"}</p>` : ""}
      ...
      ${project.type === "music" ? `
        <div class="result-box">
          ...
          ${renderMusicAnalysisHtml(project.config?.geminiAnalysis || project.config?.analysis || null)}
        </div>

        <div class="result-box">
          <div class="result-box-head"><h3>Idée de clip Gemini</h3></div>
          ${renderMusicClipIdeasHtml(project.config?.geminiClipIdeas || project.config?.clipIdeas || null)}
        </div>

        <div class="result-box">
          <div class="result-box-head"><h3>Plan de montage IA</h3></div>
          ${renderMusicMontagePlanHtml(project.config?.montagePlan || project.config?.plan || null)}
        </div>

        <div class="result-box">
          <div class="result-box-head"><h3>Sous-titres IA</h3></div>
          ${renderSubtitlesHtml(project.config?.subtitles || null)}
        </div>
      ` : ""}
...
function bindMusicProject() {
  ...
  const includeSubtitlesInput = document.getElementById("musicIncludeSubtitles");
  ...
  const syncDraft = () => {
    ...
    state.temp.musicDraft = {
      ...
      includeSubtitles: !!includeSubtitlesInput?.checked
    };
  ...
  includeSubtitlesInput?.addEventListener("change", syncDraft);
...
async function syncMusicAiToExistingProject() {
  ...
  await projectPut({
    ...
    config: {
      ...
      includeSubtitles: !!state.temp.musicDraft.includeSubtitles,
      subtitles: state.temp.musicDraft.includeSubtitles ? (state.temp.musicSubtitles || null) : null,
      geminiAnalysis: state.temp.musicAnalysis || null,
      geminiClipIdeas: state.temp.musicClipIdeas || null,
      montagePlan: state.temp.musicMontagePlan || null,
      analysis: state.temp.musicAnalysis || null,
      clipIdeas: state.temp.musicClipIdeas || null,
      plan: state.temp.musicMontagePlan || null
    }
  });
...
async function generateMusicMetadata() {
  ...
  try {
    ...
    await syncMusicAiToExistingProject();
    render();
    showToast("Métadonnées générées.");
  } catch (error) {
    console.error(error);
    const localMeta = buildLocalMusicMeta(title, draft, state.temp.musicAnalysis, state.temp.musicClipIdeas);
    state.temp.musicMetaGeneral = localMeta.general;
...
async function prepareMusicProjectWithBackend() {
  const draft = state.temp.musicDraft;
  const title = safeText(draft.name) || "Projet musique";
  const audioFile = state.temp.musicAudioFile;

  if (!audioFile) {
    throw new Error("Ajoute une musique avant le pilotage IA.");
  }
  ...
  formData.append("style", draft.style || "social");
  formData.append("tone", draft.style || "social");
  formData.append("voiceStyle", "");
  formData.append("mode", draft.mode || "video");
  formData.append("aspectRatio", draft.aspectRatio || "vertical");
  formData.append("mediaSourceMode", draft.mediaSourceMode || "single");
  formData.append("allowedBlocks", JSON.stringify(getDraftBlocks(draft)));
  formData.append("candidates", JSON.stringify(candidates));
  formData.append("includeClipIdeas", "true");
  formData.append("forceGeminiSelection", "true");
  formData.append("includeSubtitles", draft.includeSubtitles ? "true" : "false");
  ...
  formData.append("lyricsText", "");

  const result = await postForm("/api/project/prepare", formData);
  return result;
}
...
async function pilotMusicProjectWithAi() {
  ...
  try {
    ...
    try {
      prepared = await prepareMusicProjectWithBackend();
    } catch (error) {
      console.warn("Prepare backend indisponible, fallback ancien flux :", error);
    }

    if (prepared?.ok) {
      state.temp.musicAnalysis = prepared.analysis || state.temp.musicAnalysis || null;
      state.temp.musicClipIdeas = prepared.clipIdeas || state.temp.musicClipIdeas || null;
      ...
      state.temp.musicSubtitles = draft.includeSubtitles ? (prepared.subtitles || null) : null;

      let preparedSelectedIds = uniqueIds(
        prepared?.plan?.selectedMediaIds ||
        prepared?.selectedIds ||
        []
      );

      const allCandidates = mapMediaCandidatesForAi("music", draft);

      if (!preparedSelectedIds.length) {
        preparedSelectedIds = chooseLocalDiverseMediaCandidates("music", draft, allCandidates, 6).map((item) => item.id);
      }

      if (!preparedSelectedIds.length) {
        throw new Error("Aucune sélection de médias obtenue.");
      }

      const selectedCandidates = allCandidates.filter((item) => preparedSelectedIds.includes(item.id));

      let plan = prepared.plan || null;
      if (!plan || !Array.isArray(plan.selectedMediaIds) || !plan.selectedMediaIds.length) {
        try {
          const forcedPlan = await postJson("/api/montage/plan", {
            projectType: "music",
            title,
            ...
            lyricsText: draft.includeSubtitles ? "" : "",
            subtitleStartSec: Number(draft.start || 0),
            subtitleEndSec: Number(draft.start || 0) + Number(draft.targetDuration || 30)
          });

          plan = forcedPlan?.plan || null;
          if (!state.temp.musicMetaGeneral && forcedPlan?.general) {
            state.temp.musicMetaGeneral = safeText(forcedPlan.general || "");
          }
          if (!state.temp.musicMetaShorts && forcedPlan?.shorts) {
            state.temp.musicMetaShorts = safeText(forcedPlan.shorts || "").slice(0, 100);
          }
          if (draft.includeSubtitles && forcedPlan?.subtitles) {
            state.temp.musicSubtitles = forcedPlan.subtitles;
          }
        } catch (error) {
          console.warn("Plan forcé indisponible, fallback local :", error);
        }
      }
      ...
      if (!plan) {
        plan = {
          ...
          selectedMediaIds: preparedSelectedIds,
          timeline: preparedSelectedIds.map((mediaId, index) => {
            const itemDuration = Number(draft.targetDuration || 30) / Math.max(preparedSelectedIds.length, 1);
            const start = Number((index * itemDuration).toFixed(3));
            const end = index === preparedSelectedIds.length - 1
              ? Number(draft.targetDuration || 30)
              : Number(((index + 1) * itemDuration).toFixed(3));
            ...
            return {
              ...
              transition: "fade",
              effect: draft.mode === "image" ? "zoom" : "clean"
            };
          })
        };
      }

      plan.selectedMediaIds = uniqueIds(plan.selectedMediaIds?.length ? plan.selectedMediaIds : preparedSelectedIds);
      state.temp.musicMontagePlan = plan;
      state.temp.musicDraft.selectedMediaIds = plan.selectedMediaIds;

      await saveMusicDraft();
      await syncMusicAiToExistingProject();

      if (draft.montageMode === "auto") {
        const project = await createMusicProject({ openResult: false, silent: true });
    ...
    if (!state.temp.musicMetaGeneral) {
      await generateMusicMetadata();
    }
    ...
    try {
      ...
      selectedMediaIds = Array.isArray(selection.selectedIds) && selection.selectedIds.length
        ? uniqueIds(selection.selectedIds)
        : [];
    } catch (error) {
      console.warn("Sélection IA indisponible, fallback local diversifié :", error);
    }

    if (!selectedMediaIds.length) {
      selectedMediaIds = chooseLocalDiverseMediaCandidates("music", draft, candidates, 6).map((item) => item.id);
    }
    ...
    let planRes = null;
    try {
      planRes = await postJson("/api/montage/plan", {
        projectType: "music",
        title,
        aspectRatio: draft.aspectRatio,
        style: draft.style,
      ...
      console.warn("Plan API indisponible, fallback local:", error);
    }
    ...
    const defaultPlan = {
      ...
      effectStyle: draft.mode === "image" ? "zoom" : "clean",
      selectedMediaIds,
      timeline: selectedMediaIds.map((mediaId, index) => {
        const itemDuration = Number(draft.targetDuration || 30) / Math.max(selectedMediaIds.length, 1);
        const start = Number((index * itemDuration).toFixed(3));
        const end = index === selectedMediaIds.length - 1
          ? Number(draft.targetDuration || 30)
          : Number(((index + 1) * itemDuration).toFixed(3));
    ...
    const plan = planRes?.plan || defaultPlan;
    plan.selectedMediaIds = uniqueIds(plan.selectedMediaIds?.length ? plan.selectedMediaIds : selectedMediaIds);

    state.temp.musicMontagePlan = plan;
    state.temp.musicDraft.selectedMediaIds = plan.selectedMediaIds;

    if (draft.includeSubtitles && planRes?.subtitles) {
      state.temp.musicSubtitles = planRes.subtitles;
    } else if (!draft.includeSubtitles) {
      state.temp.musicSubtitles = null;
    }

    if (planRes?.general || planRes?.shorts) {
      state.temp.musicMetaGeneral = safeText(planRes.general || state.temp.musicMetaGeneral || "");
      state.temp.musicMetaShorts = safeText(planRes.shorts || state.temp.musicMetaShorts || "").slice(0, 100);
    } else if (!state.temp.musicMetaGeneral) {
      const localMeta = buildLocalMusicMeta(title, draft, state.temp.musicAnalysis, state.temp.musicClipIdeas);
      state.temp.musicMetaGeneral = localMeta.general;
      state.temp.musicMetaShorts = localMeta.shorts;
    }
...
async function createMusicProject(options = {}) {
  const { openResult = true, silent = false } = options;
  const draft = state.temp.musicDraft;
  ...
  if (!state.temp.musicMetaGeneral && state.temp.musicAnalysis) {
    await generateMusicMetadata();
  }
  ...
  const project = {
    ...
    config: {
      ...
      selectedMediaIds: uniqueIds(draft.selectedMediaIds || []),
      includeSubtitles: !!draft.includeSubtitles,
      subtitles: draft.includeSubtitles ? (state.temp.musicSubtitles || null) : null,
      geminiAnalysis: state.temp.musicAnalysis || null,
      geminiClipIdeas: state.temp.musicClipIdeas || null,
      montagePlan: state.temp.musicMontagePlan || null,
      analysis: state.temp.musicAnalysis || null,
...
async function loadMusicProjectIntoDraft(project) {
  state.temp.musicDraft = {
    ...
    selectedMediaIds: uniqueIds(project.config?.selectedMediaIds || getPlanSelectedIds(project.config?.montagePlan || project.config?.plan || null)),
    includeSubtitles: !!project.config?.includeSubtitles
  };

  state.temp.musicAnalysis = project.config?.geminiAnalysis || project.config?.analysis || null;
  state.temp.musicClipIdeas = project.config?.geminiClipIdeas || project.config?.clipIdeas || null;
  state.temp.musicMontagePlan = project.config?.montagePlan || project.config?.plan || null;
...
async function renderProjectVideo(projectId) {
  ...
  if (!linkedMedia.length) {
    showToast(project.type === "music"
      ? "Aucun média validé. Lance le pilotage IA ou sélectionne tes médias."
      : "Ajoute au moins un média au projet.");
    return;
  }
  ...
  try {
    ...
    const montagePlan = project.config?.montagePlan || project.config?.plan || null;
...
async function deleteMediaEverywhere(mediaId) {
  ...
  for (const project of state.cache.projects) {
    ...
    let nextPlan = project.config?.montagePlan || project.config?.plan || null;
