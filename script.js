...
function sourceModeHelp(mode) {
  ...
  return "Gemini peut choisir lui-même les blocs compatibles.";
}
...
function buildDiversifiedCandidatePool(projectKind, draft, count = 12) {
  ...
  for (const item of shuffled) {
    const block = normalizeBlock(item.block).toLowerCase();
    ...
    <section class="screen">
      ...
      <div class="action-grid">
        ...
        <button class="action-card" data-action="go-route" data-route="musicProject">
          ...
          <span class="action-desc">Audio + Gemini + montage vidéo</span>
        </button>
...
function musicProjectTemplate() {
  ...
  return panel(
    "Montage musique",
    "Gemini analyse la musique, choisit les médias, crée les métadonnées et prépare les sous-titres.",
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="media-preview-card">
          ...
          ${state.temp.musicAudioFile?.size > MAX_DIRECT_GEMINI_AUDIO_BYTES ? `<p class="small-note">Fichier supérieur à 20 Mo : l’analyse directe peut être ignorée. Utilise surtout "Préparer le projet complet".</p>` : ""}
        </div>
        ...
        <div class="sticky-actions">
          ...
          <button id="musicIdeasBtn" class="secondary-btn" type="button">${state.temp.musicIdeasLoading ? "Idées..." : "Créer idée de clip"}</button>
          <button id="musicMetaBtn" class="secondary-btn" type="button">${state.temp.musicMetaLoading ? "Méta..." : "Créer les métadonnées"}</button>
          <button id="musicPilotBtn" class="secondary-btn" type="button">${state.temp.musicPilotLoading ? "Préparation..." : "Préparer le projet complet"}</button>
...
function speechProjectTemplate() {
  ...
  return panel(
    "Voix IA",
    "OpenAI génère la voix. Le backend prépare les sous-titres et les métadonnées.",
    `
      <form id="speechProjectForm" class="stack-form">
        ...
        <div class="sticky-actions">
          <button id="speechGenerateCompleteBtn" class="secondary-btn" type="button">${state.temp.speechGenerating ? "Génération..." : "Générer la voix + préparer le projet"}</button>
  ...
  return panel(
    ...
    `
      ...
      ${project.type === "music" ? `
        ...
        <div class="result-box">
          <div class="result-box-head"><h3>Plan de montage Gemini</h3></div>
...
async function analyzeMusicWithGemini() {
  ...
  try {
    if (audioFile.size > MAX_DIRECT_GEMINI_AUDIO_BYTES) {
      throw new Error("Fichier supérieur à 20 Mo pour l’analyse directe.");
    }
    ...
    state.temp.musicAnalysis = normalizeGeminiObject(result.analysis) || buildFallbackMusicAnalysis(title, draft);
    ...
    await saveMusicDraft();
    render();
    showToast(error.message || "Analyse locale utilisée.");
  } finally {
...
async function generateMusicClipIdeas() {
  ...
  try {
    ...
    state.temp.musicClipIdeas = normalizeGeminiObject(result.result) || buildFallbackClipIdeas(title, draft, state.temp.musicAnalysis);
    await saveMusicDraft();
    await syncMusicAiToExistingProject();
    render();
    ...
    await saveMusicDraft();
    render();
    showToast("Idée locale utilisée.");
  } finally {
...
async function prepareMusicProjectComplete() {
  const draft = state.temp.musicDraft;
  const audioFile = state.temp.musicAudioFile;
  const title = safeText(draft.name) || "Projet musique";
  ...
  try {
    ...
    state.temp.musicAnalysis = normalizeGeminiObject(prepared.analysis) || state.temp.musicAnalysis || buildFallbackMusicAnalysis(title, draft);
    state.temp.musicClipIdeas = normalizeGeminiObject(prepared.clipIdeas) || state.temp.musicClipIdeas || buildFallbackClipIdeas(title, draft, state.temp.musicAnalysis);
    ...
    const usableCandidates = selectedCandidates.length ? selectedCandidates : buildDiversifiedLocalCandidates(candidates, 6);
    ...
    render();
    showToast("Projet complet préparé.");
  } catch (error) {
    console.error(error);
