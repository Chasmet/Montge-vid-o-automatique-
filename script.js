...
function getMusicProjectStatus(draft) {
  const audioOk = !!state.temp.musicAudioFile;
  const compatibleMedia = filterMediaForDraft("music", draft);
  const compatibleCount = compatibleMedia.length;
  const selectedCount = (draft.selectedMediaIds || []).length;
  const selectedCompatibleCount = compatibleMedia.filter((item) => (draft.selectedMediaIds || []).includes(item.id)).length;

  let warning = "";
  if (!audioOk) {
    warning = "Ajoute une musique pour continuer.";
  } else if (!compatibleCount) {
    warning = `Aucun média compatible avec ${aspectRatioLabel(draft.aspectRatio)} dans cette source. Change le format, le bloc ou ajoute des médias.`;
  } else if (draft.montageMode === "manual" && selectedCount > 0 && !selectedCompatibleCount) {
    warning = "Les médias sélectionnés ne sont pas compatibles avec les réglages actuels.";
  }
  ...
  return {
    ...
    warning,
    canAnalyze: audioOk,
    canAutoClip: audioOk && compatibleCount > 0,
    canManualProject: audioOk && compatibleCount > 0
  };
}
...
function renderMusicMontagePlanHtml(plan) {
  if (!plan) {
    return `<div class="empty-state">Aucun plan de montage pour le moment.</div>`;
  }
...
function openMediaViewer(mediaId) {
  ...
  info.innerHTML = `
    <div>Type : ${escapeHtml(media.mediaType)}</div>
  ...
  return `
    <section class="screen">
      ...
      <div class="action-grid">
        ...
        <button class="action-card" data-action="open-library" data-mode="music" data-type="video">
          <span class="action-icon">🎬</span>
...
function musicProjectTemplate() {
  ...
  const status = getMusicProjectStatus(draft);

  return panel(
    "Montage musique",
    "Ajoute la musique, choisis les médias puis lance soit le mode automatique, soit le mode manuel.",
    `
      <div class="result-box">
        <div class="result-box-head"><h3>État du projet</h3></div>
        <p class="small-note">Audio : ${status.audioOk ? "OK" : "manquant"}</p>
        <p class="small-note">Format : ${aspectRatioLabel(draft.aspectRatio)}</p>
        <p class="small-note">Source : ${sourceModeLabel(draft.mediaSourceMode)}</p>
        <p class="small-note">Médias compatibles : ${status.compatibleCount}</p>
        <p class="small-note">Médias sélectionnés : ${status.selectedCount}</p>
        ${status.warning ? `<p class="small-note" style="color:#ff8f8f;">${escapeHtml(status.warning)}</p>` : `<p class="small-note" style="color:#9be7af;">Projet prêt.</p>`}
      </div>
      ...
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="sticky-actions">
          <button id="musicAnalyzeBtn" class="secondary-btn" type="button" ${!status.canAnalyze || state.temp.musicAnalyzing ? "disabled" : ""}>
            ${state.temp.musicAnalyzing ? "Analyse..." : "Analyser la musique"}
          </button>

          <button id="musicIdeasBtn" class="secondary-btn" type="button" ${!status.canAnalyze || state.temp.musicIdeasLoading ? "disabled" : ""}>
            ${state.temp.musicIdeasLoading ? "Création..." : "Créer idée de clip"}
          </button>

          <button id="musicPilotBtn" class="primary-btn" type="button" ${!status.canAutoClip || state.temp.musicPilotLoading ? "disabled" : ""}>
            ${state.temp.musicPilotLoading ? "Création..." : "Créer le clip automatiquement"}
          </button>

          <button class="secondary-btn" type="submit" ${!status.canManualProject ? "disabled" : ""}>
...
function resultTemplate() {
  ...
  const renderError = project.config?.renderError || "";
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        ${renderError ? `<p class="small-note" style="color:#ff8f8f;">${escapeHtml(renderError)}</p>` : ""}
      </div>
...
async function saveMusicProjectRecord() {
  const draft = state.temp.musicDraft;
  const name = safeText(draft.name) || `Montage musique ${new Date().toLocaleDateString("fr-FR")}`;
  const audioFile = state.temp.musicAudioFile;

  if (!audioFile) {
    throw new Error("Ajoute une musique principale.");
  }
  ...
  const compatibleMedia = filterMediaForDraft("music", draft);
  if (!compatibleMedia.length) {
    throw new Error("Aucun média compatible pour créer le projet.");
  }
  ...
  const existingProject = draft.id ? state.cache.projects.find((p) => p.id === draft.id) : null;

  const project = {
    id: existingProject?.id || uid("project"),
    owner: state.profile,
    type: "music",
    name,
...
async function pilotMusicProjectWithAi() {
  ...
  try {
    ...
    if (!candidates.length) {
      throw new Error("Aucun média compatible pour le clip automatique.");
    }
    ...
    if (!selectedCandidates.length) {
      throw new Error("L’IA n’a retenu aucun média.");
    }
    ...
    state.route = "result";
    render();
    showToast("Pilotage terminé, création du clip en cours...");
...
async function createMusicProject() {
  try {
    ...
    console.error(error);
    showToast(error.message || "Impossible de créer le projet.");
  }
}
