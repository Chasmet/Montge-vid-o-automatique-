...
const DB_NAME = "montage-ia-mobile-v9";
const DB_VERSION = 1;
...
function modeLabel(value) {
  return value === "image" ? "Images" : "Vidéos";
}
...
function musicUsageKey(profile) {
  return `music_usage_history_${profile}`;
}

function defaultMusicUsageHistory() {
  return {
    recentProjects: [],
    recentFirstFour: [],
    statsByMediaId: {}
...
function normalizeMusicUsageHistory(data) {
  const fallback = defaultMusicUsageHistory();
  if (!data || typeof data !== "object") return fallback;

  const recentProjects = Array.isArray(data.recentProjects)
    ? data.recentProjects.map((item) => Array.isArray(item) ? item.filter(Boolean) : []).filter((item) => item.length)
    : [];

  const recentFirstFour = Array.isArray(data.recentFirstFour)
    ? data.recentFirstFour.map((item) => Array.isArray(item) ? item.filter(Boolean).slice(0, 4) : []).filter((item) => item.length)
    : [];

  const statsByMediaId = data.statsByMediaId && typeof data.statsByMediaId === "object"
    ? data.statsByMediaId
    : {};

  return {
    recentProjects: recentProjects.slice(0, 6),
    recentFirstFour: recentFirstFour.slice(0, 6),
    statsByMediaId
  };
}

async function loadMusicUsageHistory() {
  if (!state.profile) return;
  const raw = await kvGet(musicUsageKey(state.profile), defaultMusicUsageHistory());
  state.temp.musicUsageHistory = normalizeMusicUsageHistory(raw);
}

async function saveMusicUsageHistory() {
  if (!state.profile) return;
  await kvSet(musicUsageKey(state.profile), normalizeMusicUsageHistory(state.temp.musicUsageHistory));
}

async function rememberMusicSelectionUsage(mediaIds = []) {
  const ids = uniqueArray((mediaIds || []).filter(Boolean));
  if (!ids.length) return;

  const history = normalizeMusicUsageHistory(state.temp.musicUsageHistory);
  history.recentProjects.unshift(ids);
  history.recentProjects = history.recentProjects.slice(0, 6);

  const firstFour = ids.slice(0, 4);
  history.recentFirstFour.unshift(firstFour);
  history.recentFirstFour = history.recentFirstFour.slice(0, 6);

  ids.forEach((id, index) => {
    const current = history.statsByMediaId[id] || {};
    history.statsByMediaId[id] = {
      usedCount: safeNumber(current.usedCount, 0) + 1,
      firstFourCount: safeNumber(current.firstFourCount, 0) + (index < 4 ? 1 : 0),
      lastUsedAt: nowISO()
    };
  });

  state.temp.musicUsageHistory = history;
  await saveMusicUsageHistory();
}
...
function mergePlanStylesWithLocalTimeline(serverPlan, orderedCandidates, draft) {
  const local = buildLocalMontagePlanFromCandidates(
    orderedCandidates,
    Number(draft?.targetDuration || 30),
    draft?.mode || "video"
  );
  const normalized = normalizeGeminiObject(serverPlan);

  return {
    transitionStyle: safeText(normalized?.transitionStyle) || local.transitionStyle,
    effectStyle: safeText(normalized?.effectStyle) || local.effectStyle,
    selectedMediaIds: local.selectedMediaIds,
    timeline: local.timeline
  };
}

function getVariationConfig(level) {
  const normalized = normalizeVariationLevelFront(level);
  if (normalized === "low") {
    return {
      recentProjectPenalty: 28,
      recentPositionPenalty: 55,
      sameBlockPenalty: 14,
      newBlockBonus: 8,
      unusedBonus: 18,
      usedCountPenalty: 8,
      recentFirstFourBonusPenalty: 18
    };
  }

  if (normalized === "high") {
    return {
      recentProjectPenalty: 55,
      recentPositionPenalty: 120,
      sameBlockPenalty: 22,
      newBlockBonus: 22,
      unusedBonus: 40,
      usedCountPenalty: 14,
      recentFirstFourBonusPenalty: 40
    };
  }

  return {
    recentProjectPenalty: 40,
    recentPositionPenalty: 85,
    sameBlockPenalty: 18,
    newBlockBonus: 16,
    unusedBonus: 28,
    usedCountPenalty: 11,
    recentFirstFourBonusPenalty: 26
  };
}

function scoreMusicCandidateForPosition(candidate, chosen = [], history, draft) {
  const config = getVariationConfig(draft?.variationLevel || "medium");
  const stats = history?.statsByMediaId?.[candidate.id] || {};
  const usedCount = safeNumber(stats.usedCount, 0);
  const firstFourCount = safeNumber(stats.firstFourCount, 0);
  const position = chosen.length;
  const candidateBlock = normalizeBlock(candidate.block || "Vrac");
  const chosenBlocks = chosen.map((item) => normalizeBlock(item.block || "Vrac"));

  let score = 100;

  score += usedCount === 0 ? config.unusedBonus : 0;
  score -= usedCount * config.usedCountPenalty;

  if (position < 4) {
    score -= firstFourCount * Math.max(8, Math.floor(config.recentFirstFourBonusPenalty / 2));
  }

  (history?.recentProjects || []).forEach((projectIds, index) => {
    if (!Array.isArray(projectIds)) return;
    if (projectIds.includes(candidate.id)) {
      score -= index === 0 ? config.recentProjectPenalty : Math.max(10, config.recentProjectPenalty - index * 10);
    }
  });

  (history?.recentFirstFour || []).forEach((firstIds, index) => {
    if (!Array.isArray(firstIds)) return;

    if (position < 4 && firstIds[position] === candidate.id) {
      score -= index === 0 ? config.recentPositionPenalty : Math.max(18, config.recentPositionPenalty - index * 18);
    }

    if (position < 4 && firstIds.includes(candidate.id)) {
      score -= index === 0 ? config.recentFirstFourBonusPenalty : Math.max(8, config.recentFirstFourBonusPenalty - index * 6);
    }
  });

  const sameBlockCount = chosenBlocks.filter((block) => block === candidateBlock).length;
  score -= sameBlockCount * config.sameBlockPenalty;

  if (chosenBlocks.length && chosenBlocks[chosenBlocks.length - 1] === candidateBlock) {
    score -= Math.max(8, Math.floor(config.sameBlockPenalty * 0.8));
  }

  if (!chosenBlocks.includes(candidateBlock)) {
    score += config.newBlockBonus;
  }

  return score;
}

function getManualLockedMusicCandidates(draft, allCandidates = []) {
  const selectedIds = Array.isArray(draft?.selectedMediaIds) ? draft.selectedMediaIds : [];
  return selectedIds
    .map((id) => allCandidates.find((item) => item.id === id))
    .filter(Boolean);
}

function buildIntelligentMusicSelection(draft, allCandidates = [], desiredCount = 6) {
  const history = normalizeMusicUsageHistory(state.temp.musicUsageHistory);
  const locked = getManualLockedMusicCandidates(draft, allCandidates);
  const lockedIds = new Set(locked.map((item) => item.id));

  const targetCount = Math.min(
    allCandidates.length,
    Math.max(desiredCount, locked.length)
  );

  let chosen = [...locked];
  let pool = allCandidates.filter((item) => !lockedIds.has(item.id));

  while (chosen.length < targetCount && pool.length) {
    const ranked = [...pool]
      .map((candidate) => ({
        candidate,
        score: scoreMusicCandidateForPosition(candidate, chosen, history, draft)
      }))
    ...
    chosen.push(next);
    pool = pool.filter((item) => item.id !== next.id);
  }

  if (!chosen.length) {
    chosen = buildDiversifiedLocalCandidates(allCandidates, desiredCount);
  }
...
function buildMusicSelectionReasoning(draft, lockedCount, finalCandidates = []) {
  const variation = variationLevelLabel(draft?.variationLevel || "medium").toLowerCase();
  const count = finalCandidates.length;
  const mode = modeLabel(draft?.mode || "video").toLowerCase();

  if (lockedCount > 0) {
    return `${lockedCount} média(s) imposé(s) par toi en tête, puis complément IA avec variation ${variation}. Type actif : ${mode}. Total retenu : ${count}.`;
  }

  return `Sélection IA diversifiée avec variation ${variation}. Type actif : ${mode}. Total retenu : ${count}.`;
}
...
function renderMusicModeTabs(activeMode) {
  return `
    <div class="segmented-group" style="margin-bottom:12px;">
      <p class="segment-title">Type de médias</p>
      <div class="segmented-buttons">
        <button
          class="segment-btn ${activeMode === "image" ? "active" : ""}"
          type="button"
          data-action="set-music-mode-tab"
          data-mode="image"
        >
          Images
        </button>
        <button
          class="segment-btn ${activeMode === "video" ? "active" : ""}"
          type="button"
          data-action="set-music-mode-tab"
          data-mode="video"
        >
...
function renderMusicMontagePlanHtml(planInput) {
  ...
  const selectedNames = selectedIds.map((id) => {
    const media = state.cache.media.find((item) => item.id === id);
    return media?.fileName || id;
  });
  ...
  return `
    <div class="card-list">
      ...
      <div class="project-card"><h3 class="project-title">Médias retenus</h3><div class="small-note">${selectedNames.length ? selectedNames.map((name) => escapeHtml(name)).join(" • ") : "—"}</div></div>
      ${
        timeline.length
          ? `
            <div class="project-card">
...
function renderMusicPreparationSummaryHtml(preparedInput, draft) {
  ...
  return `
    <div class="card-list">
      ...
      <div class="project-card">
        <h3 class="project-title">Type actif</h3>
        <div class="small-note">${modeLabel(draft?.mode || "video")}</div>
      </div>
    ...
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="media-preview-card">
          ...
          <p class="small-note">Si tu choisis toi-même des médias et leur ordre, le pilotage IA les garde en tête puis complète le reste.</p>
        </div>
        ...
        <div class="result-box">
          ...
          ${renderMusicModeTabs(draft.mode)}

          <div class="project-card" style="margin-bottom:12px;">
            <h3 class="project-title">Résumé rapide</h3>
            <div class="small-note">
              Type actif : ${modeLabel(draft.mode)}<br>
              Médias choisis manuellement : ${selectedIds.length}<br>
              L’IA gardera cet ordre puis complétera le reste si besoin.
            </div>
          </div>
        ...
        <div class="result-box">
          <div class="result-box-head">
            <h3>Ordre prioritaire utilisateur</h3>
          </div>
          ${renderSelectedMediaOrder("music", selectedIds)}
          <p class="small-note">Les médias dans cette liste restent en tête. L’IA complète ensuite avec d’autres médias si besoin.</p>
        </div>
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        <p class="small-note">Type actif : ${modeLabel(project.config?.mode || "video")}</p>
...
function bindMusicProject() {
  ...
  const syncDraft = () => {
    ...
    state.temp.musicDraft = {
      ...
      mode: state.temp.musicDraft.mode || "video",
...
async function prepareMusicProjectComplete(options = {}) {
  ...
  try {
    const candidates = buildDiversifiedCandidatePool("music", draft, 24);
    ...
    const desiredCount = Math.min(
      candidates.length,
      Math.max(6, safeNumber((draft.selectedMediaIds || []).length, 0))
    );

    const smartSelection = buildIntelligentMusicSelection(draft, candidates, desiredCount);
    const lockedCount = smartSelection.lockedCount;
    const finalOrderedCandidates = smartSelection.selected;
    ...
    formData.append("candidatesJson", JSON.stringify(finalOrderedCandidates));
    ...
    formData.append("lockedPrefixCount", String(lockedCount));
    ...
    state.temp.musicMontagePlan = mergePlanStylesWithLocalTimeline(
      prepared?.plan,
      finalOrderedCandidates,
      draft
    );
    ...
    const selectionReasoning = buildMusicSelectionReasoning(
      draft,
      lockedCount,
      finalOrderedCandidates
    );
    ...
    state.temp.musicPreparation = {
      ...(state.temp.musicPreparation || {}),
      source: {
        analysis: prepared?.source?.analysis || (prepared?.analysis ? "gemini_or_local" : "local"),
        clipIdeas: prepared?.source?.clipIdeas || "gemini_or_local",
        mediaSelection: lockedCount > 0 ? "user_locked_prefix_plus_ai" : (prepared?.source?.mediaSelection || "local_diversified"),
        meta: prepared?.source?.meta || "gemini_or_local",
        subtitles: prepared?.source?.subtitles || (state.temp.musicSubtitles ? "prepared" : "none"),
        plan: "local_timeline_prioritized"
      },
      selectionReasoning,
      warnings: prepared?.warnings || {}
    };

    await rememberMusicSelectionUsage(state.temp.musicDraft.selectedMediaIds);
    await saveMusicDraft();
    await syncMusicAiToExistingProject();
    ...
    const fallbackCandidatesAll = buildDiversifiedCandidatePool("music", draft, 24);
    const smartFallback = buildIntelligentMusicSelection(
      draft,
      fallbackCandidatesAll,
      Math.min(fallbackCandidatesAll.length, Math.max(6, (draft.selectedMediaIds || []).length || 0))
    );
    const fallbackOrderedCandidates = smartFallback.selected;

    state.temp.musicMontagePlan = mergePlanStylesWithLocalTimeline(
      null,
      fallbackOrderedCandidates,
      draft
    );
    state.temp.musicDraft.selectedMediaIds = state.temp.musicMontagePlan.selectedMediaIds || [];
    state.temp.musicSubtitles = null;
    ...
    state.temp.musicPreparation = {
      source: {
        ...
        mediaSelection: smartFallback.lockedCount > 0 ? "user_locked_prefix_plus_local_ai" : "local_diversified",
      ...
      selectionReasoning: buildMusicSelectionReasoning(draft, smartFallback.lockedCount, fallbackOrderedCandidates)
    };
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "set-music-mode-tab") {
    const nextMode = target.dataset.mode === "image" ? "image" : "video";
    if (state.temp.musicDraft.mode === nextMode) return;

    state.temp.musicDraft.mode = nextMode;
    resetMusicForCriteriaChange();
    state.temp.musicDraft.mode = nextMode;
    await saveMusicDraft();
    render();
  ...
  if (action === "move-project-media") {
    ...
    if (kind === "music") {
      ...
      if (state.temp.musicMontagePlan?.selectedMediaIds?.length) {
        ...
        state.temp.musicMontagePlan.timeline = buildLocalMontagePlanFromCandidates(
          state.temp.musicDraft.selectedMediaIds
            .map((id) => {
              const media = state.cache.media.find((item) => item.id === id);
            ...
            .filter(Boolean),
          Number(state.temp.musicDraft.targetDuration || 30),
          state.temp.musicDraft.mode || "video"
        ).timeline;
      }
  ...
  if (action === "clear-cache") {
    ...
    state.temp.musicSubtitles = null;
    state.temp.musicPreparation = null;
    state.temp.musicUsageHistory = defaultMusicUsageHistory();
    ...
    await kvSet(musicUsageKey(state.profile), defaultMusicUsageHistory());
    await hydrateCache();
    await saveSpeechDraft();
    await saveMusicDraft();
    await saveSoraDraft();
