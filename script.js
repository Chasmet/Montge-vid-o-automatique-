...
const PROFILE_ACCENTS = {
  ...
  user1: {
    border: "rgba(14,165,164,0.55)",
    bg: "linear-gradient(180deg, rgba(14,165,164,0.18), rgba(15,23,42,0.88))",
    badgeBg: "rgba(14,165,164,0.20)",
    badgeText: "#7dd3fc"
  },
  user2: {
    ...
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="sticky-actions">
          <button id="musicPilotBtn" class="primary-btn" type="button">
            ${state.temp.musicPilotLoading ? "Pilotage..." : "Pilotage IA complet"}
          </button>
          <button class="secondary-btn" type="submit">Créer ou mettre à jour le projet</button>
        </div>

        <details class="result-box">
          <summary style="cursor:pointer;font-weight:700;">Options manuelles</summary>
          <div style="margin-top:14px;">
            <div class="prompt-actions">
              <button id="musicAnalyzeBtn" class="secondary-btn" type="button">${state.temp.musicAnalyzing ? "Analyse..." : "Mettre à jour l’analyse"}</button>
              <button id="musicIdeasBtn" class="secondary-btn" type="button">${state.temp.musicIdeasLoading ? "Lecture..." : "Relire le clip"}</button>
              <button id="musicMetaBtn" class="secondary-btn" type="button">${state.temp.musicMetaLoading ? "Méta..." : "Rafraîchir la méta"}</button>
            </div>
...
function bindMusicProject() {
  ...
  [nameInput, durationSelect, styleSelect, montageModeSelect, aspectRatioSelect, variationSelect].forEach((el) => {
