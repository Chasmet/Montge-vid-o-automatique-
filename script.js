...
function pickDiverseMedia(items = [], maxCount = 6) {
  ...
  for (const item of clean) {
    const key = normalizeBlock(item.block || "Vrac");
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(item);
  }

  const groups = [...grouped.values()].sort((a, b) => b.length - a.length);
  const picked = [];

  while (picked.length < Math.min(maxCount, clean.length)) {
    let added = false;
    ...
    for (const group of groups) {
      ...
      added = true;
      if (picked.length >= Math.min(maxCount, clean.length)) break;
    }
...
function buildMusicGeminiContext(draft) {
  ...
  return [
    ...
    `sous_titres=${draft.enableSubtitles ? "oui" : "non"}`
  ].join(" | ");
}
...
async function loadMusicDraft() {
  ...
  state.temp.musicDraft.enableSubtitles = !!state.temp.musicDraft.enableSubtitles;
  ...
  return panel(
    ...
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="result-box">
          <div class="result-box-head"><h3>Pilotage IA</h3></div>
          <label class="project-card" style="display:flex;align-items:center;gap:12px;">
            <input id="musicEnableSubtitles" type="checkbox" ${draft.enableSubtitles ? "checked" : ""} />
            <div>
              <div class="project-title">Sous-titres auto</div>
              <div class="small-note">Seulement si tu coches cette case.</div>
            </div>
          </label>
        </div>
  ...
  return panel(
    ...
    `
      ...
      <div class="result-box">
        ...
        <p class="small-note">Sous-titres auto : ${project.config?.enableSubtitles ? "Oui" : "Non"}</p>
...
function bindMusicProject() {
  ...
  const enableSubtitlesInput = document.getElementById("musicEnableSubtitles");
  ...
  const syncDraft = () => {
    ...
    state.temp.musicDraft = {
      ...
      enableSubtitles: !!enableSubtitlesInput?.checked
    };
  ...
  enableSubtitlesInput?.addEventListener("change", syncDraft);
...
async function syncMusicAiToExistingProject() {
  ...
  await projectPut({
    ...
    config: {
      ...
      enableSubtitles: !!state.temp.musicDraft.enableSubtitles
    }
  });
...
async function prepareMusicProjectWithBackend() {
  ...
  formData.append("lyricsText", draft.enableSubtitles ? "AUTO" : "");
...
async function pilotMusicProjectWithAi() {
  ...
  try {
    ...
    if (prepared?.ok) {
      ...
      state.temp.musicMontagePlan = prepared.plan || state.temp.musicMontagePlan || null;
      ...
      const preparedSelectedIds =
        prepared?.plan?.selectedMediaIds ||
        prepared?.selectedIds ||
        [];

      if (Array.isArray(preparedSelectedIds) && preparedSelectedIds.length) {
        state.temp.musicDraft.selectedMediaIds = uniqueById(
          preparedSelectedIds.map((id) => ({ id }))
        ).map((item) => item.id);
      }
    ...
    try {
      const selection = await postGeminiJson("/api/gemini/select-media", {
        title: safeText(draft.name) || "Projet musique",
      ...
      selectedMediaIds = Array.isArray(selection.selectedIds) && selection.selectedIds.length
        ? uniqueById(selection.selectedIds.map((id) => ({ id }))).map((item) => item.id)
        : [];
    } catch (error) {
    ...
    if (!selectedMediaIds.length) {
      selectedMediaIds = pickDiverseMedia(
        candidates.map((item) => ({ ...item, createdAt: Date.now() })),
        6
      ).map((item) => item.id);
    }
    ...
    if (planRes?.general || planRes?.shorts) {
      ...
      const localMeta = buildLocalMusicMeta(
        safeText(draft.name) || "Projet musique",
        draft,
...
async function loadMusicProjectIntoDraft(project) {
  state.temp.musicDraft = {
    ...
    enableSubtitles: !!project.config?.enableSubtitles
  };
