...
function speechProjectTemplate() {
  ...
  return panel(
    "Projet speech",
    "Texte + vraie voix IA + métadonnées en un seul bouton.",
    `
...
function bindSpeechProject() {
  ...
  [nameInput, textInput, voiceSelect, toneSelect, speedSelect, modeSelect].forEach((el) => {
  ...
  try {
    const [metaResult, speechResponse] = await Promise.all([
      postJson("/api/meta/generate", {
        ...
        tone: draft.tone,
        voice: draft.voice,
        notes: "Format prêt à copier, sans écrire Titre Description Hashtags, 5 hashtags max."
      }),
