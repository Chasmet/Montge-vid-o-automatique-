...
const DB_NAME = "montage-ia-mobile-final-video-v1";
const DB_VERSION = 1;
...
function getSelectedOrFallbackMedia(project) {
  ...
  if (!items.length) {
    ...
    items = state.cache.media.filter((m) => m.bucket === bucket);
  }
  ...
  return `
    <div class="media-grid">
      ${items.map((item) => {
        ...
        return `
          <button type="button" class="media-card" data-action="toggle-project-media" data-kind="${kind}" data-mode="${mode}" data-media-id="${item.id}">
...
async function getBlobDuration(blob, type = "audio") {
...
function musicProjectTemplate() {
  ...
  return panel(
    "Projet musique",
    "V1 avec vrai rendu vidéo via le backend.",
    `
      <form id="musicProjectForm" class="stack-form">
        ...
        <div class="result-box">
          ...
          <p class="small-note">${selectedIds.length} média(s) lié(s)</p>
...
function speechProjectTemplate() {
  ...
  return panel(
    "Projet speech",
    "Voix + métadonnées + vrai rendu vidéo.",
    `
...
function projectsTemplate() {
  return panel(
    "Mes projets",
    "Projets, reprise, rendu vidéo et téléchargement.",
    `
...
function resultTemplate() {
  ...
  const linkedMediaCount = (project.config?.selectedMediaIds || []).length;
  ...
  let renderLabel = "Créer le montage vidéo";
  if (renderStatus === "processing") renderLabel = "Rendu en cours...";
  if (renderStatus === "done") renderLabel = "Refaire le montage vidéo";
  if (isBusy) renderLabel = "Rendu en cours...";

  return panel(
    "Résultat du projet",
    "Projet local + rendu vidéo V1.",
    `
      <div class="result-box">
        <div class="result-box-head">
          <h3>Infos projet</h3>
        ...
        <p class="small-note">Médias liés : ${linkedMediaCount}</p>
...
async function createMusicProject() {
  ...
  const project = {
    ...
    mainMeta: `${name}\nMontage ${draft.mode === "image" ? "image" : "vidéo"} ${draft.style} prêt à poster.\n${hashtags.join(" ")}`,
    shortMeta: `${name}\n${hashtags.slice(0, 2).join(" ")}`.slice(0, 99)
  };
  ...
  try {
    const [metaResult, speechResponse] = await Promise.all([
      postJson("/api/meta/generate", {
        ...
        notes: `Style vocal ${findVoiceLabel(draft.voiceStyle)}. Réponse en un seul bloc, sans écrire Titre Description Hashtags, 5 hashtags max.`
      }),
...
async function createSpeechProject() {
  ...
  const project = {
    ...
    mainMeta: state.temp.speechMetaGeneral || `${name}\nVoix ${findVoiceLabel(draft.voiceStyle)}, ton ${draft.tone}, montage ${draft.mode}.\n${fallbackHashtags.join(" ")}`,
    shortMeta: state.temp.speechMetaShorts || `${name}\n${fallbackHashtags.slice(0, 2).join(" ")}`.slice(0, 99)
  };
...
async function renderProjectVideo(projectId) {
  ...
  if (!linkedMedia.length) {
    showToast("Ajoute ou lie au moins un média au projet.");
    return;
  }
  ...
  try {
    ...
    await projectPut(updatedProject);
    await hydrateCache();
    render();
    ...
    formData.append("montageMode", project.config?.montageMode || "auto");
    ...
    linkedMedia.forEach((item, index) => {
      formData.append(
        "media",
        item.blob,
        item.fileName || `media_${index + 1}.${item.mediaType === "image" ? "jpg" : "mp4"}`
      );
    });
    ...
    await projectPut(finalProject);
    await hydrateCache();
    state.currentResultId = finalProject.id;
    render();
    showToast("Vidéo prête.");
  } catch (error) {
    console.error(error);

    const projectAfterError = {
      ...project,
      updatedAt: nowISO(),
    ...
    await projectPut(projectAfterError);
    await hydrateCache();
    render();
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "delete-media") {
    ...
    state.cache.projects = state.cache.projects.map((project) => {
      const selected = (project.config?.selectedMediaIds || []).filter((id) => id !== mediaId);
      const finalVideoMediaId = project.config?.finalVideoMediaId === mediaId ? null : project.config?.finalVideoMediaId;
