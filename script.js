...
function speechProjectTemplate() {
  ...
  return panel(
    "Voix IA",
    "OpenAI génère la voix. Gemini prépare les sous-titres et les métadonnées.",
    `
      <form id="speechProjectForm" class="stack-form">
        ...
        <div class="media-preview-card">
          ...
          <p class="small-note">OpenAI génère la voix. Gemini complète le reste.</p>
        </div>
...
async function analyzeMusicWithGemini() {
  ...
  try {
    ...
    state.temp.musicAnalysis = result.analysis || buildFallbackMusicAnalysis(title, draft);
    ...
    await saveMusicDraft();
    render();
    showToast("Analyse locale utilisée.");
  } finally {
...
async function generateMusicClipIdeas() {
  ...
  try {
    ...
    state.temp.musicClipIdeas = result.result || buildFallbackClipIdeas(title, draft, state.temp.musicAnalysis);
    await saveMusicDraft();
    await syncMusicAiToExistingProject();
    render();
...
async function prepareMusicProjectComplete() {
  ...
  try {
    ...
    const prepared = await fetch(`${BACKEND_BASE_URL}/api/project/prepare`, {
      method: "POST",
      body: formData
    }).then((r) => parseApiResponse(r, "Préparation impossible"));

    state.temp.musicPreparation = prepared || null;
    state.temp.musicAnalysis = prepared.analysis || state.temp.musicAnalysis || buildFallbackMusicAnalysis(title, draft);
    state.temp.musicClipIdeas = prepared.clipIdeas || state.temp.musicClipIdeas || buildFallbackClipIdeas(title, draft, state.temp.musicAnalysis);
...
async function createMusicProject(options = {}) {
  ...
  if (!state.temp.musicMontagePlan) {
    await prepareMusicProjectComplete();
  }
