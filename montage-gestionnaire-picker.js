(() => {
  ...
  const SESSION_KEY = "montage_gestionnaire_import_session_v2";
  ...
  function base64ToFile(base64, name, mimeType) {
    ...
    return new File([bytes], name || "fichier-gestionnaire", { type: mimeType || "application/octet-stream" });
  }

  function listGestionnaireFiles() {
    if (!hasBridge()) return [];
    try { return JSON.parse(GestionnaireLibrary.listFilesJson() || "[]"); } catch { return []; }
  }

  function getSession() {
    if (!state.temp.gestionnaireImportSession) {
      state.temp.gestionnaireImportSession = { blocks: {}, keys: [] };
    }
    return state.temp.gestionnaireImportSession;
  }

  function rememberImport(block, key, mediaId) {
    const session = getSession();
    session.blocks[block] = session.blocks[block] || [];
    if (!session.keys.includes(key)) session.keys.push(key);
    if (!session.blocks[block].includes(mediaId)) session.blocks[block].push(mediaId);
  }

  function importKey(info, block) {
    return `${block}::${info.relativePath || info.uri || info.name || "file"}`;
  }

  function alreadyImported(info, block) {
    const key = importKey(info, block);
    const session = getSession();
    if (session.keys.includes(key)) return true;
    return (state.cache.media || []).some(item => item.sourceKey === key || item.gestionnaireKey === key);
  }

  function getImportedBlocksSummary() {
    const groups = new Map();
    for (const item of state.cache.media || []) {
      if (!item.temporary && item.sourceApp !== "gestionnaire") continue;
      const block = item.block || "Vrac";
      if (!groups.has(block)) groups.set(block, { count: 0, size: 0 });
      const g = groups.get(block);
      g.count += 1;
      g.size += Number(item.size || item.blob?.size || 0);
    }
    return groups;
  }
  ...
  async function ensureBlock(blockName) {
    ...
    if (!window.state) return clean;

    state.customBlocks = Array.isArray(state.customBlocks) ? state.customBlocks : [];
    const exists = state.customBlocks.some(b => String(b).toLowerCase() === clean.toLowerCase());
    const defaultExists = typeof DEFAULT_MEDIA_BLOCKS !== "undefined" && DEFAULT_MEDIA_BLOCKS.some(b => String(b).toLowerCase() === clean.toLowerCase());
  ...
  async function useAsMusic(info) {
    ...
    if (!file.type.startsWith("audio/")) return toast("Choisis un fichier audio pour la musique principale.");
    if (state.temp.musicAudioUrl) { try { URL.revokeObjectURL(state.temp.musicAudioUrl); } catch {} }
    ...
    toast("Audio pioché dans Gestionnaire.");
    closePanel();
    render();
  }
  ...
  async function useAsTemporaryVisual(info, blockName = "Vrac") {
    ...
    if (alreadyImported(info, block)) return "duplicate";
    ...
    const key = importKey(info, block);
    const id = `gestion_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    rememberImport(block, key, id);
    return true;
  }

  async function importFolderAsBlock(folder, button = null) {
    ...
    if (!visualFiles.length) return toast("Ce dossier ne contient pas d’image ou de vidéo.");
    ...
    if (state.temp?.musicDraft) {
      state.temp.musicDraft.mediaSourceMode = "single";
      state.temp.musicDraft.primaryBlock = block;
      state.temp.musicDraft.allowedBlocks = Array.from(new Set([...(state.temp.musicDraft.allowedBlocks || []), block]));
      if (typeof saveMusicDraft === "function") await saveMusicDraft();
    }

    if (typeof render === "function") render();
    setTimeout(() => window.dispatchEvent(new CustomEvent("android-shared-files-ready")), 120);
    closePanel();
    toast(`${block} prêt : ${added} nouveau${added > 1 ? "x" : ""}, ${skipped} déjà présent${skipped > 1 ? "s" : ""}. Les autres blocs restent actifs.`);
  }

  function openFolderView(folder) {
    const files = folder.items || [];
    const title = folder.folderPath || folder.name || "Dossier";
    const listHtml = files.length ? files.map((file, index) => `
      ...
      </div>`).join("") : `<p style="color:#cbd5e1;">Dossier vide.</p>`;
    ...
    renderShell(`
      ...
      <button id="gestionImportFolder" style="width:100%;border:0;border-radius:14px;padding:13px;margin-top:12px;background:#22c55e;color:#06230f;font-weight:900;">Importer / compléter ce bloc</button>
      <div style="margin-top:12px;">${listHtml}</div>
    `);

    document.getElementById("gestionBackFolders").onclick = openPanel;
    document.getElementById("gestionImportFolder").onclick = (event) => importFolderAsBlock(folder, event.currentTarget).catch(() => toast("Import du dossier impossible."));
    document.querySelectorAll("[data-use-music]").forEach(button => button.onclick = () => useAsMusic(files[Number(button.dataset.useMusic)]).catch(() => toast("Impossible d’utiliser ce fichier.")));
    document.querySelectorAll("[data-use-media]").forEach(button => button.onclick = () => useAsTemporaryVisual(files[Number(button.dataset.useMedia)], folder.name).then(ok => { if (ok) { toast(ok === "duplicate" ? "Déjà présent dans ce bloc." : "Média ajouté sans effacer les autres blocs."); render(); } }).catch(() => toast("Impossible d’utiliser ce média.")));
  }
  ...
  function renderShell(contentHtml) {
    ...
    panel.id = PANEL_ID;
    panel.style.cssText = `position:fixed;inset:0;z-index:999999;background:rgba(2,6,23,.82);color:#fff;font-family:system-ui,sans-serif;display:flex;align-items:flex-end;`;
    panel.innerHTML = `
      <div style="width:100%;max-height:84vh;overflow:auto;background:#0f172a;border-radius:22px 22px 0 0;padding:16px;border-top:1px solid #334155;box-shadow:0 -18px 40px rgba(0,0,0,.45);">
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px;">
          <div><h2 style="margin:0;font-size:20px;">Piocher dans Gestionnaire</h2><p style="margin:5px 0 0;color:#94a3b8;font-size:13px;">Import additif : un bloc ajouté n’efface pas les autres.</p></div>
  ...
  function openPanel() {
    if (!hasBridge()) return toast("Le lien Gestionnaire n’est disponible que dans l’APK.");
    const files = listGestionnaireFiles();
    if (!files.length) return openGestionnaireImporter();
    const folders = groupByFolder(files);
    const summaries = getImportedBlocksSummary();

    const foldersHtml = folders.length ? folders.map((folder, index) => {
      ...
      const imported = summaries.get(block)?.count || 0;
      return `
        <div style="border:1px solid #334155;border-radius:16px;padding:12px;margin-top:10px;background:#111827;">
          ...
          <span style="display:block;color:#94a3b8;font-size:12px;margin-top:4px;">${folder.items.length} fichiers · ${visualCount} médias · ${audioCount} audio · déjà importés : ${imported}</span>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px;">
            ...
            <button data-import-folder="${index}" style="border:0;border-radius:12px;padding:11px;background:#22c55e;color:#06230f;font-weight:900;">Importer / compléter</button>
          </div>
        </div>`;
    }).join("") : `<p style="color:#cbd5e1;">Aucun dossier exploitable trouvé.</p>`;

    renderShell(`<button id="gestionOpenImporter" style="width:100%;border:0;border-radius:14px;padding:13px;background:#2563eb;color:white;font-weight:900;margin-bottom:12px;">➕ Importer un dossier depuis le téléphone</button><div>${foldersHtml}</div>`);
    document.getElementById("gestionOpenImporter").onclick = openGestionnaireImporter;
    document.querySelectorAll("[data-open-folder]").forEach(button => button.onclick = () => openFolderView(folders[Number(button.dataset.openFolder)]));
    document.querySelectorAll("[data-import-folder]").forEach(button => button.onclick = (event) => importFolderAsBlock(folders[Number(button.dataset.importFolder)], event.currentTarget).catch(() => toast("Import du dossier impossible.")));
  }
  ...
  function installButton() {
    ...
    btn.style.cssText = `position:fixed;right:14px;bottom:82px;z-index:99998;border:0;border-radius:999px;padding:12px 14px;background:#2563eb;color:white;font-weight:900;box-shadow:0 12px 28px rgba(0,0,0,.35);`;
