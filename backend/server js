...
async function geminiAnalyzeAudio({
  ...
  const prompt = `
Analyse cet audio pour préparer un clip musical court.

Règles :
- Réponds en JSON valide uniquement
- Français uniquement
- Pas de markdown
- Sois concret et exploitable
...
`.trim();
  ...
  const analysis = result.json || { raw: result.raw || "Analyse vide." };
...
- garder un ordre final cohérent
...
- Priorité à la variété des blocs
- Retourne les IDs dans l'ordre final souhaité
...
async function geminiMetaGenerate({
  ...
  const prompt = `
Crée des métadonnées réseaux sociaux utiles pour un projet créatif.
...
- Pas de markdown
- Pas de texte autour
- Description claire, accrocheuse et naturelle
- Hashtags utiles et propres
- Shorts doit faire maximum 100 caractères
...
`.trim();
  ...
  const payload = {
    general: safeText(result.json?.general || ""),
    shorts: safeText(result.json?.shorts || "").slice(0, 100),
    keywords: Array.isArray(result.json?.keywords) ? result.json.keywords : [],
...
async function geminiSubtitlesFromText({
  ...
  const prompt = `
Découpe ce texte en sous-titres pour une vidéo.

Règles :
- JSON valide uniquement
- Français uniquement
- Pas de markdown
- Respecte la plage temps demandée
- Segments lisibles
...
function ensureMusicPlanUsable(plan, selectedCandidates, fallbackOptions = {}) {
  ...
  return {
    ...
    selectedMediaIds:
      Array.isArray(plan.selectedMediaIds) && plan.selectedIds
  ...
  try {
    ...
    if (isGeminiConfigured()) {
      try {
        ...
        if (geminiMeta?.general || geminiMeta?.shorts) {
          meta = {
