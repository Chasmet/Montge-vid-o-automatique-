...
const GEMINI_BOOT_TIMEOUT_MS = Number(process.env.GEMINI_BOOT_TIMEOUT_MS || 15000);
const GEMINI_RETRIES = Number(process.env.GEMINI_RETRIES || 1);
const MAX_GEMINI_AUDIO_BYTES = Number(
  process.env.MAX_GEMINI_AUDIO_BYTES || 20 * 1024 * 1024
);
...
const ENABLE_GEMINI_META = envBool(process.env.ENABLE_GEMINI_META, true);
const ENABLE_GEMINI_SUBTITLES = envBool(process.env.ENABLE_GEMINI_SUBTITLES, true);
const ENABLE_GEMINI_SORA = envBool(process.env.ENABLE_GEMINI_SORA, true);
const ENABLE_PROJECT_CACHE = envBool(process.env.ENABLE_PROJECT_CACHE, true);

const CACHE_TTL_MS = Number(process.env.CACHE_TTL_MS || 10 * 60 * 1000);
const MAX_RENDER_MEDIA = Number(process.env.MAX_RENDER_MEDIA || 4);
...
async function warmGeminiServer(id = "gemini") {
  ...
  try {
    const response = await fetchWithTimeout(
      ...
      { method: "GET" },
      GEMINI_BOOT_TIMEOUT_MS
    );

    log(id, "GEMINI WAKE OK");
    return response.ok;
  } catch (error) {
    log(id, "GEMINI WAKE FAIL", error?.message || "erreur");
    return false;
  }
}

async function postGeminiWithRetry(path, options = {}, id = "gemini") {
  let lastError = null;

  await warmGeminiServer(id);

  for (let attempt = 0; attempt <= GEMINI_RETRIES; attempt += 1) {
    try {
      const response = await fetchWithTimeout(
        `${GEMINI_SERVER_URL}${path}`,
        options,
        GEMINI_TIMEOUT_MS
      );
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error || payload?.message || `Erreur appel Gemini ${path}.`
        );
      }
      ...
      log(
        id,
        "GEMINI RETRY",
        `${path} tentative=${attempt + 1} erreur=${error?.message || "erreur"}`
      );
...
async function callGeminiAnalyzeAudio({
  ...
  if (buffer.length > MAX_GEMINI_AUDIO_BYTES) {
    log(
      id,
      "GEMINI ANALYZE SKIP",
      `audio trop lourd ${Math.round(buffer.length / 1024 / 1024)}Mo`
    );
    return {
      ok: false,
      skipped: true,
  ...
  const payload = await postGeminiWithRetry(
...
async function callGeminiMetaGenerate({
  ...
  if (!ENABLE_GEMINI_META || !isGeminiConfigured()) {
...
async function callGeminiSubtitlesFromText({
  ...
  if (!ENABLE_GEMINI_SUBTITLES || !isGeminiConfigured()) {
    return { ok: false, skipped: true, subtitles: null };
  }

  const cacheKey = sha1(
    JSON.stringify({
      kind: "gemini-subtitles-from-text",
      lyricsText,
      lyricsApprox,
      startSec,
      endSec
  ...
  log(id, "GEMINI SUBTITLES OK", subtitles?.enabled ? "enabled" : "none");
...
async function callGeminiSoraPrompts({
  title = "Projet vidéo",
  start = 0,
  end = 10,
  segmentDuration = 10,
  ...
  if (!ENABLE_GEMINI_SORA || !isGeminiConfigured()) {
    return { ok: false, skipped: true, prompts: [] };
  }

  const cacheKey = sha1(
    JSON.stringify({
      kind: "gemini-sora-prompts",
      title,
      start,
      end,
      segmentDuration,
  ...
  const cached = getCache(cacheKey);
  if (cached) {
    log(id, "GEMINI SORA CACHE HIT", title);
    return { ok: true, source: "cache", prompts: cached };
  }

  log(id, "GEMINI SORA START", title);

  const payload = await postGeminiWithRetry(
    "/api/gemini/sora-prompts",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
  ...
  log(id, "GEMINI SORA OK", `${title} / ${prompts.length}`);
...
app.get("/api/health", (_req, res) => {
  res.json({
    ...
    geminiMetaEnabled: ENABLE_GEMINI_META,
    geminiSubtitlesEnabled: ENABLE_GEMINI_SUBTITLES,
    geminiSoraEnabled: ENABLE_GEMINI_SORA,
...
app.post("/api/meta/generate", async (req, res) => {
  ...
  try {
    ...
    log(id, "META REQUEST", `${title} / ${projectType}`);
    ...
    try {
      ...
      return res.json({
        ok: true,
        source: geminiMeta.source || "gemini",
        general: geminiMeta.general || "",
        shorts: (geminiMeta.shorts || "").slice(0, 100),
        keywords: geminiMeta.keywords || [],
        hashtags: geminiMeta.hashtags || []
      });
    } catch (error) {
      log(id, "META GEMINI FAIL -> LOCAL", error?.message || "erreur");
    }
...
app.post("/api/subtitles/from-text", async (req, res) => {
  ...
  try {
    ...
    try {
      const geminiSubtitles = await callGeminiSubtitlesFromText({
        lyricsText,
        lyricsApprox,
        startSec,
        endSec,
      ...
      if (geminiSubtitles?.subtitles) {
        return res.json({
          ok: true,
          source: geminiSubtitles.source || "gemini",
          subtitles: geminiSubtitles.subtitles
        });
      }
    } catch (error) {
      log(id, "SUBTITLES GEMINI FAIL -> LOCAL", error?.message || "erreur");
    }
...
app.post("/api/project/prepare", upload.single("audio"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (analyzed?.ok && analyzed?.analysis && Object.keys(analyzed.analysis).length) {
        analysis = analyzed.analysis;
        analysisSource = analyzed?.source || "gemini";
      } else {
        analysis = buildFallbackMusicAnalysis(title, style);
        analysisSource = analyzed?.source || "local_fallback";
      }
    } catch (error) {
    ...
    try {
      ...
      shorts = safeText(geminiMeta.shorts || "").slice(0, 100);
      metaSource = geminiMeta.source || "gemini";
    } catch (error) {
      metaError = error?.message || "Meta Gemini impossible.";
      const meta = buildGeminiBasedMeta({
        title,
        projectType,
        style,
    ...
    try {
      ...
      subtitles = geminiSubtitles?.subtitles || null;
      subtitlesSource = subtitles?.enabled ? geminiSubtitles.source || "gemini" : "none";
    } catch (error) {
...
app.post("/api/sora/prompts", async (req, res) => {
  ...
  try {
    ...
    try {
      const geminiPrompts = await callGeminiSoraPrompts({
        title,
        start,
        end,
        segmentDuration,
      ...
      if (Array.isArray(geminiPrompts?.prompts) && geminiPrompts.prompts.length) {
        return res.json({
          ok: true,
          source: geminiPrompts.source || "gemini",
          prompts: geminiPrompts.prompts
        });
      }
    } catch (error) {
      log(id, "SORA GEMINI FAIL -> LOCAL", error?.message || "erreur");
    }
