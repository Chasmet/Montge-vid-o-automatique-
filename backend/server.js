...
const ENABLE_GEMINI_CLIP_IDEAS = envBool(process.env.ENABLE_GEMINI_CLIP_IDEAS, false);
...
function compactCandidatesForGemini(candidates = []) {
  return (Array.isArray(candidates) ? candidates : [])
    ...
    .map((item, index) => ({
      ...
      block:
        safeText(item.block || item.collection || item.category || item.group || ""),
...
function chooseCandidatesByIds(candidates = [], selectedIds = []) {
  ...
  const selected = (Array.isArray(candidates) ? candidates : []).filter((item) =>
...
function shouldUseGeminiSelection({
  ...
  const mediaTypeSet = new Set(
    candidates.map((item) => safeText(item?.mediaType || item?.type || "video")).filter(Boolean)
  );

  if (Array.isArray(allowedBlocks) && allowedBlocks.length > 1) return true;
  if (blockSet.size > 1) return true;
  if (mediaTypeSet.size > 1) return true;
  if (safeText(mediaSourceMode) !== "single" && candidates.length > 4) return true;
...
function buildLocalMontagePlan({
  ...
  const computedEffectStyle =
    ...
    cleanCandidates.every((item) => safeText(item.mediaType) === "image")
      ? "zoom"
      : "clean");
...
app.post("/api/project/prepare", upload.single("audio"), async (req, res) => {
  ...
  try {
    ...
    const includeClipIdeas = parseBooleanish(req.body?.includeClipIdeas, false);
    const forceGeminiSelection = parseBooleanish(req.body?.forceGeminiSelection, false);
    ...
    let mediaSelectionSource = "local_skip";
    ...
    if (
      ...
      try {
        ...
        if (selectedIds.length) {
          ...
          mediaSelectionSource = `${mediaSelectionSource}_empty`;
        }
      } catch (error) {
