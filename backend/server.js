...
function buildScalePadFilter(index) {
  return `[${index}:v]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,fps=30,setsar=1,format=yuv420p[v${index}]`;
}
...
async function renderImageMontage({
  ...
  const perImage = Math.max(targetDurationSec / Math.max(imagePaths.length, 1), 0.5);

  const args = ["-y"];

  imagePaths.forEach((img) => {
    args.push("-loop", "1", "-t", String(perImage), "-i", img);
  });
  ...
  imagePaths.forEach((_img, index) => {
    filters.push(buildScalePadFilter(index));
    labels.push(`[v${index}]`);
  });

  filters.push(`${labels.join("")}concat=n=${imagePaths.length}:v=1:a=0[vout]`);
...
async function renderVideoMontage({
  ...
  for (const filePath of videoPaths) {
    baseDurations.push(await probeDuration(filePath));
  }
  ...
  while (total < targetDurationSec && cursor < 120) {
    const index = cursor % videoPaths.length;
    const duration = Math.max(baseDurations[index] || 1, 1);
    sequence.push({
  ...
  if (!sequence.length) {
    throw new Error("Aucune vidéo exploitable pour le rendu.");
  }

  const extraPad = Math.max(targetDurationSec - total, 0);
  const args = ["-y"];
  ...
  sequence.forEach((_item, index) => {
    filters.push(buildScalePadFilter(index));
    labels.push(`[v${index}]`);
  });

  filters.push(`${labels.join("")}concat=n=${sequence.length}:v=1:a=0[vcat]`);

  if (extraPad > 0) {
    filters.push(`[vcat]tpad=stop_mode=clone:stop_duration=${extraPad.toFixed(3)}[vout]`);
  } else {
    filters.push(`[vcat]copy[vout]`);
  }

  args.push(
    "-filter_complex", filters.join(";"),
    "-map", "[vout]",
    "-map", `${sequence.length}:a:0`,
    "-t", String(targetDurationSec),
    "-shortest",
    "-c:v", "libx264",
    "-preset", "veryfast",
