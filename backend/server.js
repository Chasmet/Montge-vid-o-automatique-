...
const OPENAI_MODEL = process.env.OPENAI_MODEL || process.env.OPENAI_TEXT_MODEL || "gpt-4.1-mini";
const TTS_MODEL = process.env.TTS_MODEL || process.env.OPENAI_TTS_MODEL || "gpt-4o-mini-tts";
...
function buildFallbackMeta({
  ...
  const visual = safeText(clipIdeas?.visualStyle || analysis?.visualUniverse || style || "cinematique");
  const voice = safeText(voiceStyle || "");
  const typeTag = slugTag(projectType || "media") || "media";
  const moodTag = slugTag(mood || "creatif") || "creatif";
  const visualTag = slugTag(visual || "cinematique") || "cinematique";

  const general = `${cleanTitle}
${projectType === "speech"
    ? `Voix IA ${voice || "naturelle"}, ambiance ${mood}.`
    : `Création ${projectType} ${style}, ambiance ${mood}.`
  } ${visual ? `Univers visuel : ${visual}.` : ""} Format prêt pour ${mode === "video" ? "vidéo" : "image"}.
#${typeTag} #montageia #${moodTag} #${visualTag} #mobile`;

  const shorts = `${cleanTitle}
#${typeTag} #montageia #${moodTag}`.slice(0, 100);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        reject(new Error(`FFmpeg a échoué: ${label}${stderrTail ? ` | ${stderrTail}` : ""}`));
      }
    });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      log(id, "RENDER START", `${title} / ${mode} / ${aspectRatio} / ${finalDuration}s / media=${mediaFiles.length}`);
