...
function getRatioConfig(aspectRatio) {
...
function buildVideoFilter(ratio) {
  return [
    `scale=${ratio.width}:${ratio.height}:force_original_aspect_ratio=decrease`,
    `pad=${ratio.width}:${ratio.height}:(ow-iw)/2:(oh-ih)/2`,
    "setsar=1",
    "format=yuv420p"
  ].join(",");
}
...
function getSegmentDuration(totalSeconds, totalItems, index) {
  const base = totalSeconds / totalItems;
  if (index === totalItems - 1) {
    const used = base * index;
    return Math.max(0.1, totalSeconds - used);
  }
...
async function createImageSegment(inputPath, outputPath, durationSec, ratio, id, label) {
  const vf = buildVideoFilter(ratio);
...
async function createVideoSegment(inputPath, outputPath, durationSec, ratio, id, label) {
  const vf = buildVideoFilter(ratio);
...
async function concatSegmentsCopy(segmentPaths, outputPath, id) {
  ...
  await runFfmpeg(args, id, "concat segments copy");
}
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const aspectRatio = safeText(req.body?.aspectRatio) || "horizontal";
      ...
      const ratio = getRatioConfig(aspectRatio);
      log(id, "RENDER RATIO", `${ratio.label} ${ratio.width}x${ratio.height}`);
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        const dur = getSegmentDuration(finalDuration, count, i);
        ...
        if (mode === "video") {
          await createVideoSegment(media.path, out, dur, ratio, id, `segment video ${i + 1}`);
        } else {
          await createImageSegment(media.path, out, dur, ratio, id, `segment image ${i + 1}`);
        }

        await removePathQuietly(media.path);
      }

      if (segmentPaths.length === 1) {
        await fsp.copyFile(segmentPaths[0], silentVideoPath);
        log(id, "CONCAT SKIP", "1 seul segment");
      } else {
        await concatSegmentsCopy(segmentPaths, silentVideoPath, id);
      }
