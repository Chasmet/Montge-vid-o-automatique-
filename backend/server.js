...
function parseGeminiTaggedAnalysis(rawText) {
  const text = normalizeMultilineText(rawText);
  if (!text) return null;

  const regex = new RegExp(`(^|\\n)(${ANALYSIS_MARKERS.join("|")}):`, "g");
  const matches = [];
  let match;
  ...
  return {
    ...
    summaryShort: sections.SUMMARY_SHORT || "",
    summaryLong: sections.SUMMARY_LONG || "",
    mood: sections.MOOD || "",
    energy: sections.ENERGY || "",
    rhythm: sections.RHYTHM || "",
    visualUniverse: sections.VISUAL_UNIVERSE || "",
    emotions: splitLooseList(sections.EMOTIONS || ""),
    themes: splitLooseList(sections.THEMES || ""),
    keywords: splitLooseList(sections.KEYWORDS || ""),
    hooks: splitLooseList(sections.HOOKS || ""),
    artDirection: sections.ART_DIRECTION || "",
    clipType: sections.CLIP_TYPE || ""
  };
}

function validateAnalysisCandidate(candidate) {
  return !!(
    candidate &&
    typeof candidate === "object" &&
    safeText(candidate.summaryShort) &&
    safeText(candidate.summaryLong) &&
    safeText(candidate.mood) &&
    safeText(candidate.visualUniverse)
  );
}
...
async function callGeminiAnalysisText(parts, options = {}, id = "gemini") {
  ...
  const maxOutputTokens = Number(options.maxOutputTokens || 900);
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      ...
      const parsed = parseGeminiTaggedAnalysis(raw);

      if (!validateAnalysisCandidate(parsed)) {
        const preview = raw.replace(/\s+/g, " ").slice(0, 220);
        log(id, "GEMINI ANALYZE RAW", preview || "vide");
        throw new Error("Réponse Gemini illisible ou incomplète.");
      }
...
async function geminiAnalyzeAudio({
  ...
  const prompt = `
Tu analyses un extrait audio musical ou vocal en français.

Réponds uniquement en texte brut, sans JSON, sans markdown, sans puces.
Tu dois utiliser exactement ces balises, dans cet ordre, avec du contenu après chaque balise :
...
Règles :
- TRANSCRIPTION : paroles entendues ou approximation crédible si l'audio n'est pas parfaitement intelligible
- SUMMARY_SHORT : résumé très court
- SUMMARY_LONG : résumé riche et utile
- MOOD : ambiance dominante
- ENERGY : basse, moyenne ou haute
- RHYTHM : type de rythme
- VISUAL_UNIVERSE : univers visuel suggéré
- EMOTIONS : 3 à 5 émotions séparées par |
- THEMES : 3 à 5 thèmes séparés par |
- KEYWORDS : 4 à 8 mots-clés séparés par |
- HOOKS : 2 à 4 moments forts séparés par |
- ART_DIRECTION : direction artistique concrète
- CLIP_TYPE : type de clip conseillé en quelques mots
...
`.trim();
  ...
  const analysis = {
    transcription: compactParagraph(candidate.transcription, 700),
    summary: compactSentence(candidate.summaryShort, 180),
    detailedSummary: compactParagraph(candidate.summaryLong, 320),
    dominantMood: compactSentence(candidate.mood, 50),
    energyLevel: compactSentence(candidate.energy, 20),
    rhythmEstimate: compactSentence(candidate.rhythm, 70),
    visualUniverse: compactSentence(candidate.visualUniverse, 90),
    emotions: normalizeKeywordList(candidate.emotions, 5),
    themes: normalizeKeywordList(candidate.themes, 5),
    keywords: normalizeKeywordList(candidate.keywords, 8),
    hookMoments: normalizeSentenceList(candidate.hooks, 4, 90),
    artisticDirection: compactParagraph(candidate.artDirection, 220),
    clipType: compactSentence(candidate.clipType, 60),
    lyricsApprox: normalizeTranscriptionLines(candidate.transcription),
    sceneIdeas: [],
    editingAdvice: [],
    soraDirection: ""
  };
...
function buildSceneIdeasFromAnalysis(analysis = {}) {
  ...
  if (universe.includes("urb")) {
    ideas.push("rue, néons et marche déterminée");
  }
...
function buildEditingAdviceFromAnalysis(analysis = {}) {
  ...
  if (mood.includes("cin")) {
    advice.push("soigner les images d’ouverture et de fin");
  }
  if (mood.includes("émotion")) {
    advice.push("éviter de surcharger le montage");
  }
  if (mood.includes("sombre")) {
    advice.push("renforcer les contrastes et la profondeur");
  }
...
function buildSoraDirectionFromAnalysis(analysis = {}) {
  const parts = [
    ...
    safeText(analysis.visualUniverse)
      ? `Univers : ${analysis.visualUniverse}`
      : "",
    safeText(analysis.artisticDirection)
      ? `Direction : ${analysis.artisticDirection}`
      : "",
    analysis.hookMoments?.length
      ? `Moments forts : ${analysis.hookMoments.join(", ")}`
      : ""
  ].filter(Boolean);
...
function scoreCandidate(candidate, analysis, clipIdeas, aspectRatio, allowedBlocks) {
  ...
  for (const word of words) {
    ...
    if (slugTag(haystack).includes(word)) score += 2;
  }
...
app.post("/api/subtitles/from-text", async (req, res) => {
  ...
  try {
    ...
    log(id, "SUBTITLES LOCAL OK", subtitles.enabled ? `${subtitles.segments.length} segments` : "none");
