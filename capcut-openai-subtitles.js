/* V43 - Outil séparé CapCut : vidéo + audio séparé, OpenAI transcrit l'audio et incruste dans la vidéo */
(function () {
  const VERSION = 'V43';
  ...
  function setStatus(text) { statusText = text || ''; refreshPanel(); if (text) toast(text); }
  ...
  function injectCard() {
    ...
    card.innerHTML = `
      ...
      <div><h3>Sous-titres CapCut</h3><p>Upload vidéo + audio. OpenAI transcrit l’audio et incruste dans la vidéo.</p></div>
  ...
  function sizeMessage() {
    ...
    if (videoTooHeavy()) return `<div class="capcut-alert capcut-alert-danger"><strong>Vidéo trop lourde : ${mbText(selectedVideo)}</strong><br>Maximum app : ${MAX_VIDEO_MB} Mo. Exporte ta vidéo CapCut en 720p/1080p compressé.</div>`;
    if (videoWarning()) return `<div class="capcut-alert"><strong>Vidéo lourde : ${mbText(selectedVideo)}</strong><br>Ça peut passer, mais ce sera lent. Recommandé : moins de 180 Mo.</div>`;
    return `<div class="capcut-status">Taille vidéo OK : ${mbText(selectedVideo)}.</div>`;
  }
  ...
  function renderPanelContent() {
    ...
    const canCreateVideoWithAudio = selectedVideo && selectedAudio && !busy && !videoTooHeavy();
    const canCreateVideoAlone = selectedVideo && !busy && !videoTooHeavy();
    ...
    return `
      <div class="capcut-panel-head">
        <div>
          ...
          <p class="small-note">Workflow conseillé : upload vidéo + audio séparé. L’application utilise l’audio pour créer le SRT, puis incruste directement dans la vidéo.</p>
        </div>
      ...
      <div class="result-box">
        <div class="result-box-head"><h3>✅ Mode conseillé : vidéo + audio</h3></div>
        <label class="field"><span>Vidéo CapCut compressée</span><input id="capcutVideoInput" type="file" accept="video/*" /></label>
        <p class="small-note">Vidéo : ${fileName}${selectedVideo ? ` - ${mbText(selectedVideo)}` : ''}</p>
        ${sizeMessage()}
        ...
        <div class="prompt-actions"><button type="button" class="primary-btn" data-action="capcut-openai-audio-video" ${canCreateVideoWithAudio ? '' : 'disabled'}>${busy ? 'Traitement en cours...' : 'Créer vidéo sous-titrée avec audio OpenAI'}</button></div>
        <p class="small-note">Rapide : OpenAI ne transcrit que l’audio. FFmpeg incruste ensuite les sous-titres dans la vidéo.</p>
      </div>

      <div class="result-box">
        <div class="result-box-head"><h3>🎬 Mode secours : vidéo seule</h3></div>
        <p class="small-note">À utiliser seulement si tu n’as pas l’audio séparé. Plus long, car Render doit extraire l’audio de la vidéo.</p>
        <div class="prompt-actions"><button type="button" class="secondary-btn" data-action="capcut-openai-subtitle" ${canCreateVideoAlone ? '' : 'disabled'}>${busy ? 'Traitement en cours...' : 'Créer vidéo sous-titrée depuis vidéo seule'}</button></div>
      </div>

      <div class="result-box">
        <div class="result-box-head"><h3>⚡ Mode rapide SRT uniquement</h3></div>
        <p class="small-note">OpenAI crée seulement le fichier SRT depuis l’audio. Tu peux ensuite l’utiliser dans CapCut.</p>
      ...
      ${lastResultUrl ? `<div class="capcut-mini-result"><h3>✅ Vidéo sous-titrée prête</h3><video class="capcut-preview" controls playsinline preload="metadata" src="${lastResultUrl}"></video><div class="prompt-actions"><button type="button" class="primary-btn" data-action="capcut-download-final">Télécharger la vidéo sous-titrée</button></div></div>` : ''}
  ...
  function openPanel() {
    ensurePanelStyles(); panelOpen = true;
    let backdrop = document.getElementById('capcutOpenAiPanelBackdrop');
    if (!backdrop) {
      backdrop = document.createElement('div'); backdrop.id = 'capcutOpenAiPanelBackdrop'; backdrop.className = 'capcut-panel-backdrop'; backdrop.innerHTML = `<section class="capcut-panel" id="capcutOpenAiPanel"></section>`; document.body.appendChild(backdrop);
    }
    const panel = document.getElementById('capcutOpenAiPanel'); if (panel) panel.innerHTML = renderPanelContent();
  }
  ...
  async function saveMedia(blob, fileName) {
    ...
    const media = { id: uid('capcut_subtitled'), owner: state.profile || 'admin', bucket: 'project-video', mediaType: 'video', fileName, mimeType: 'video/mp4', size: blob.size || 0, createdAt: typeof nowISO === 'function' ? nowISO() : new Date().toISOString(), block: 'Vrac', orientation: (localStorage.getItem('capcut_subtitle_ratio') || 'vertical') === 'horizontal' ? 'horizontal' : 'vertical', tags: ['capcut', 'openai', 'sous-titres'], blob };
    await mediaPut(media); if (typeof hydrateCache === 'function') await hydrateCache(); return media;
  }
  ...
  async function createSrtFromFile(file) {
    ...
    form.append('audio', file, file.name || 'audio.mp3'); form.append('subtitleStyle', style); form.append('subtitleSyncMode', 'normal'); form.append('aspectRatio', aspectRatio); form.append('forceTimelineZero', '1'); form.append('audioStartSec', '0'); form.append('audioEndSec', '0');
    ...
    const srt = rebaseSrtToZero(data?.srt || ''); if (!looksLikeSrt(srt)) throw new Error('OpenAI a répondu, mais le SRT est invalide.'); return srt;
  }

  async function createSrtOnly() {
    if (busy) return; if (!selectedAudio) return toast('Choisis d’abord un fichier audio.');
    busy = true; setStatus('OpenAI crée le SRT depuis l’audio...');
    try { lastSrt = await createSrtFromFile(selectedAudio); setStatus('SRT OpenAI prêt.'); }
    catch (error) { console.error(error); toast(error.message || 'Erreur SRT OpenAI.'); }
    finally { busy = false; refreshPanel(); }
  }

  async function createSubtitledVideoFromEndpoint(endpoint, modeLabel) {
    if (busy) return;
    ...
    busy = true; setStatus(modeLabel);
    try {
      const style = document.getElementById('capcutSubtitleStyle')?.value || localStorage.getItem('capcut_subtitle_style') || 'classic';
      const aspectRatio = document.getElementById('capcutAspectRatio')?.value || localStorage.getItem('capcut_subtitle_ratio') || 'vertical';
      localStorage.setItem('capcut_subtitle_style', style); localStorage.setItem('capcut_subtitle_ratio', aspectRatio);
      const form = new FormData();
      form.append('video', selectedVideo, selectedVideo.name || 'video_capcut.mp4'); form.append('subtitleStyle', style); form.append('aspectRatio', aspectRatio);
      if (endpoint.includes('audio-video')) {
        if (!selectedAudio) throw new Error('Audio séparé manquant.');
        form.append('audio', selectedAudio, selectedAudio.name || 'audio_capcut.mp3');
      }
      const response = await fetch(`${BACKEND_BASE_URL}${endpoint}`, { method: 'POST', body: form });
      if (!response.ok) { let msg = 'Sous-titrage CapCut impossible.'; try { const data = await response.json(); msg = data?.error || msg; } catch {} throw new Error(msg); }
      const finalBlob = await response.blob(); if (!finalBlob || finalBlob.size < 1000) throw new Error('Render a renvoyé une vidéo vide.');
      lastResultBlob = finalBlob; revokeUrl(lastResultUrl); lastResultUrl = URL.createObjectURL(finalBlob);
      const outputName = `${safeName(selectedVideo.name.replace(/\.[^.]+$/, ''))}_openai_sous_titres.mp4`;
      await saveMedia(finalBlob, outputName); setStatus('Vidéo CapCut sous-titrée prête.');
    } catch (error) { console.error(error); toast(error.message || 'Erreur outil CapCut OpenAI.'); }
    finally { busy = false; refreshPanel(); }
  }

  document.addEventListener('change', (event) => {
    if (event.target?.id === 'capcutVideoInput') { const file = event.target.files?.[0] || null; selectedVideo = file; revokeUrl(selectedVideoUrl); selectedVideoUrl = file ? URL.createObjectURL(file) : ''; revokeUrl(lastResultUrl); lastResultUrl = ''; lastResultBlob = null; statusText = file && file.size > MAX_VIDEO_BYTES ? `Vidéo trop lourde : ${mbText(file)}. Exporte en 720p/1080p compressé.` : ''; refreshPanel(); }
    if (event.target?.id === 'capcutAudioInput') { selectedAudio = event.target.files?.[0] || null; lastSrt = ''; statusText = selectedAudio ? `Audio prêt : ${selectedAudio.name} - ${mbText(selectedAudio)}.` : ''; refreshPanel(); }
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'capcut-openai-audio-video') await createSubtitledVideoFromEndpoint('/api/capcut/openai-subtitles-audio-video', 'Upload vidéo + audio. OpenAI transcrit l’audio, puis Render incruste...');
    if (action === 'capcut-openai-subtitle') await createSubtitledVideoFromEndpoint('/api/capcut/openai-subtitles', 'Upload vidéo vers Render. Extraction audio puis OpenAI...');
    if (action === 'capcut-create-srt-only') await createSrtOnly();
    if (action === 'capcut-download-final') { if (!lastResultBlob) return toast('Aucune vidéo prête.'); downloadBlob(lastResultBlob, `${safeName(selectedVideo?.name || 'video_capcut')}_openai_sous_titres.mp4`); }
    if (action === 'capcut-download-srt') { if (!lastSrt) return toast('Aucun SRT prêt.'); downloadBlob(new Blob([lastSrt], { type: 'text/plain;charset=utf-8' }), `${safeName(selectedAudio?.name || 'audio_capcut')}_openai.srt`); }
    if (action === 'capcut-copy-srt') { if (!lastSrt) return toast('Aucun SRT prêt.'); await navigator.clipboard.writeText(lastSrt); toast('SRT copié.'); }
  });
